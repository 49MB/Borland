...
final class KeyComparator {
  ...
  KeyComparator(int[] indexVector, DataColumn[] dataColumns,
      SortDescriptor descriptor) {
    /*
     * int count = 0;
     * 
     * if (descending != null) { for (int index = 0; index < descending.length;
     * ++index) { if (descending[index]) { ++count; } } } return count;
     */
    ...
    if (descriptor.isDescending()
        || (descending != null && descending.length < 1))
      descending = null;
  ...
  final void setIndexVector(int[] indexVector) {
    this.indexVector = indexVector;
  }
  
  private final void printIndexVector() {
    if (false) {
      for (int element : indexVector)
        Diagnostic.print(element + " ");
      Diagnostic.println("");
    }
  }
  ...
  // this is useful for repetitive compares of the same row values (sorting) and
  // for
  // compares of data rows that do not exist in the indexVector (locate row
  // values).
  //
  private final void secondaryCompare(int comparColumnCount, int dataRow1,
      int dataRow2, boolean compareRow) {
    if (comparColumnCount > 1) {
      for (int index = 1; comp == 0 && index < comparColumnCount; ++index) {
        ...
        // ! bug72202.
        //
        if (descending != null && index < descending.length
            && descending[index])
          comp = -comp;
      }
    }
  ...
  final void findPivots(int leftStart, int rightStart) {
    ...
    while (true) {
      leftPivot = firstColumn.forwardCompare(leftPivot, caseInsensitive,
          firstDescending);
      ...
      if (comp >= 0 || leftPivot == indexVector.length - 1)
        break;
    }
    
    while (true) {
      rightPivot = firstColumn.reverseCompare(rightPivot, caseInsensitive,
          firstDescending);
  ...
  void reverseLimitedCompare() {
    if (firstDescending) {
      if (caseInsensitive) {
        while (--rightPivot > -1
            && (comp = firstColumn.compareIgnoreCase(indexVector[rightPivot],
                pivotDataRow)) < 0)
          ;
      } else {
        while (--rightPivot > -1
            && (comp = firstColumn.compare(indexVector[rightPivot],
                pivotDataRow)) < 0)
          ;
      }
      comp = -comp;
  ...
  final int findInsertPoint(int step, int delta, boolean compareRow) {
    ...
    while (true) {
      ...
      if (rightPivot < 0)
        break;
      secondaryCompare(columnCount, indexVector[rightPivot], pivotDataRow,
          compareRow);
      if (comp <= 0)
        break;
    }
  ...
  private final DataColumn firstColumn;
