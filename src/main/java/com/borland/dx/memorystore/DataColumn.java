...
abstract class DataColumn {
  ...
  static final double NULL_DOUBLE = Double.MIN_VALUE;
  static final long NULL_LONG = Long.MIN_VALUE;
  static final String NULL_STRING = "\u0000\u0000\u0000"; // NORES
  ...
  // ! static final Timestamp nullTimestamp = new Timestamp(130, 0, 0, 0, 0, 0,
  // 0);
  // static final BigDecimal nullBigDecimal = new
  // BigDecimal("999999999999999999"); // NORES
  ...
  DataColumn(NullState nullState) {
    ...
    // ! Diagnostic..println("assignedMask "+Integer.toString(assignedMask,
    // 16));
    // ! Diagnostic..println("unassigndedMask "+Integer.toString(unassignedMask,
    // 16));
  ...
  final void growTo(int newLength, int lastRow) {
    Diagnostic.check(newLength >= lastRow);
  ...
  boolean fullPartialCompare(String v1, String v2, boolean caseInsensitive) {
    if (caseInsensitive) {
      v1 = v1.toUpperCase();
      v2 = v2.toUpperCase();
    }
