...
/**
 * The ProcedureResolver component is used to resolve (save) changes back to a
 * JBDC data source by calling stored procedures in that database. The stored
 * procedures must exist prior to using this component; this component will not
 * generate them. These stored procedures must meet the requirements described
 ...
 * 
 * 
 * The database property of this component must be set to the Database component
 * that this ProcedureResolver is associated with. Otherwise, a DataSetException
 * is generated.
 * 
 * 
 * 
 ...
 * Sybase users:
 * <p>
 * Stored procedures on Sybase servers are created in a "Chained" transaction
 * mode. In order to call Sybase stored procedures as part of this component,
 * the procedures must be modified to run in an unchained transaction mode. Use
 * the Sybase stored system procedure sp_procxmode to change the transaction
 * mode to either "anymore" or "unchained". See your Sybase documentation for
...
public class ProcedureResolver extends SQLResolver {
  ...
  /**
   * Instructs the Resolver to insert the current row of the DataSet into the
   * Database.
   * 
   * @param dataSet
   *          DataSet
  ...
  {
    try {
      Diagnostic.trace(Trace.DataSetSave, "ProcedureResolver inserting row "
          + dataSet);
  ...
  /**
   * Instructs the Resolver to update the current row of the DataSet in the
   * Database.
   * 
   * @param dataSet
   *          DataSet
  ...
  {
    try {
      Diagnostic.trace(Trace.DataSetSave, "ProcedureResolver updating row:  "
          + dataSet.getRow());
      Diagnostic.trace(Trace.DataSetSave, "                  updating from: "
          + oldDataRow);
  ...
  /**
   * Instructs the Resolver to delete the current row in the DataSet from the
   * Database.
   * 
   * @param dataSet
   *          DataSet
  ...
  {
    try {
      Diagnostic.trace(Trace.DataSetSave, "ProcedureResolver deleting row "
          + dataSet.getRow());
  ...
  private void checkBeforeCall(ProcedureDescriptor descriptor) /*-throws DataSetException-*/{
    if (descriptor == null || descriptor.getDatabase() == null
        || descriptor.getStrippedQueryString() == null)
  ...
  private String replaceWithNumber(String procedure, String parameter,
      int parameterValue) {
    if (procedure == null)
      return null;
    ...
    while (index > 0) {
      procedure = procedure.substring(0, index + 1) + parameterValue
          + procedure.substring(index + parameter.length());
      index = copy.lastIndexOf(parameter, index - 1);
    }
    return procedure;
  }
  
  private transient final ReadWriteRow params[];
  private transient final boolean readonly[];
