...
/**
 * The <CODE>ProcedureProvider</CODE> class provides data to the
 * {@link com.borland.dx.dataset.StorageDataSet} <CODE>StorageDataSet</CODE></A>
 * by executing the specified stored procedure through JDBC. You connect this
 * component to the <CODE>StorageDataSet</CODE> component through the <CODE>StorageDataSet's</CODE>
 * <CODE>provider</CODE> property.
 * 
 * 
 * <P>
 * This class provides the data to the <CODE>StorageDataSet</CODE> however it
 * does not attempt to make the <CODE>StorageDataSet</CODE> updatable or
 * editable; it is the developer's responsibility to ensure this prior to the
 * start of the resolution phase.
 * 
 * <P>
 * The stored procedure is expected to return a result set. For stored
 * procedures that don't return values, use either static form of the
...
public class ProcedureProvider extends JdbcProvider {
  ...
  //
  /**
   * This method can also be used for queries with an output parameter, for
   * example:
   * 
   * <PRE><CODE>     callProcedure(db1,"select sum(expense) into ? from expense_table",paramRow);
   * @param database Database
   * @param procedureSpecification String
   * @param parameters ReadWriteRow
   * <p>
   */
  public static final int callProcedure(Database database,
      String procedureSpecification, ReadWriteRow parameters)
  ...
  /**
   * Calls a stored procedure with named access to multiple rows of parameters.
   * The parameters are accessed via an optional tag of the named parameters.
   * The tag is the number of the passed <CODE>ReadWriteRow</CODE>, starting
   * with 1. For example, the following code passes the value of the 'Name'
   * column from the parameter row 'param1' as the first parameter, and the
   * value of the 'Name' column from the parameter row 'param2'.
   * 
   *<PRE>
   ...
   * 
   *<P>
   * If no tag is given, the parameter name is found by searching the parameter
   * rows from left to right. Therefore, tags can be used to differentiate
   * columns with identical names in different parameter rows.
   * 
   * @param database
   *          database
  ...
  static final int callProcedure(Database database,
      String procedureSpecification, ReadWriteRow[] parameters,
      boolean[] inputOnly)
  /*-throws DataSetException-*/
  {
    ...
    try {
      ...
      procedure.resetState(database, procedureSpecification, null, parameters,
          inputOnly);
  ...
  void cacheDataSet(StorageDataSet dataSet) /*-throws DataSetException-*/{
    ...
    if (procedureDescriptor == null
        || procedureDescriptor.getDatabase() == null
        || procedureDescriptor.getStrippedQueryString() == null)
      DataSetException.badProcedureProperties();
  ...
  /*-throws DataSetException-*/throws SQLException {
    ...
    try {
      procedure.setFetchAsNeededData(fetchAsNeededData);
      return procedure.executeQuery();
    } finally {
      procedure.setFetchAsNeededData(null);
    }
  }
  ...
  void resetState() /*-throws DataSetException-*/throws SQLException {
    ...
    if (procedureDescriptor == null || dataSet == null)
      ...
      procedure.resetState(procedureDescriptor.getDatabase(),
          procedureDescriptor.getStrippedQueryString(),
          dataSet.getMasterLink(), procedureDescriptor.getParameterRow(), null);
  }
  
  // get better error message:
  /**
   * Tests whether the data is present. This method is used when providing data
   * asynchronously to determine whether editing, resolving, and other such
   * actions should be blocked until the data is available.
   */
  @Override
