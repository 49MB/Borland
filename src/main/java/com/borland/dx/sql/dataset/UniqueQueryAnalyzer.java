...
public class UniqueQueryAnalyzer extends QueryAnalyzer {
  ...
  UniqueQueryAnalyzer(Database database, String queryString,
  ...
  public void analyze() throws SQLException, DataSetException {
    ...
    // See if query is a summary query: either a distinct query or a group by
    // query:
    if (isSummaryQuery()) {
      updatable = false;
      return;
  ...
  public void analyzeTableName() throws SQLException, DataSetException {
    ...
    columnCount = columnList.size();
    calcUniqueTableName();
    SQLElement singleTable = tableList.size() == 1 ? (SQLElement) tableList
        .elementAt(0) : null;
    ...
    if (columnCount == 1) {
      ...
      if (column == null
          || ("*".equals(column.getName()) && column.getPrefixName() == null)) {
      ...
      if (column != null) { // Expressions are stored as a null column !
        if (column.getName() == null)
          ...
          if (column.getAlias() != null)
            columnsByName.put(column.getAlias(), column);
          else
  ...
  private String getTablePrefixName(SQLElement column) {
    ...
    table = index < 0 ? new SQLElement(tableAlias, null, null, null, null)
        : (SQLElement) tableList.elementAt(index);
    if (table != null) {
  ...
  private int insertAllColumns(SQLElement table, int insertAt)
    ...
    if (schemaName == null && database.isUseSchemaName())
      schemaName = userName;
    ResultSet columns = database.getMetaData().getColumns(null, schemaName,
        tableName, "%"); // NORES
    while (columns.next()) {
    ...
    for (int tableIndex = 0; tableIndex < tableList.size() - 1; tableIndex++) {
      ...
      while (!isUnique) {
        ...
        for (int otherTableIndex = 0; otherTableIndex < tableList.size(); otherTableIndex++) {
          SQLElement otherTable = (SQLElement) tableList
              .elementAt(otherTableIndex);
  ...
  private void generateColumnsFromResultSetMetaData() throws SQLException,
      DataSetException {
  ...
  public String getTableName() {
    return tableList == null || tableList.size() != 1 ? null
        : ((SQLElement) tableList.elementAt(0)).getName();
  }
  
  public String getSchemaName() {
    return tableList == null || tableList.size() != 1 ? null
        : ((SQLElement) tableList.elementAt(0)).getPrefixName();
  }
  ...
  public void setBestRowId(Vector columns) /*-throws DataSetException-*/{
    ...
    // Check if query has been parsed (could be here if Jdbc driver supported
    // getTableName()).
    if (parser == null)
      parse();
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (!tablesByColumn.containsKey(column)) {
        Diagnostic.trace(Trace.QueryAnalyze, "Adding row id column to query: "
            + column);
    ...
    while (tmpToken.getType() == SQLToken.OTHER) {
      tokens = tmpToken;
    ...
    // If this is a "SELECT *" query, project '*' to a series of 'T1.*,T2.*,...'
    // field specifications
    while (tokens.isField() && tokens.getName().equals("*")
        && tokens.getPrefixName() == null) {
      tmpToken = tokens.getNextToken();
      Vector tables = tableList;
      int tableCount = tables.size();
      for (int index = 0; index < tableCount; index++) {
    ...
    Diagnostic.trace(Trace.QueryAnalyze, "Generating a new query: "
        + generatedQuery);
  }
  ...
  /*
   * // Parse the query. If problems parsing the query, return false // boolean
   * parse() { Diagnostic.trace(Trace.QueryAnalyze,"Invoking parser");
   * 
   * try { return super.parse(); } catch (DataSetException ex) {
   * Diagnostic.printStackTrace(ex); //! &&&&& //! do work here to catch
   * exceptions for: //! - not a SELECT query //! - malformed query (could mean
   * don't understand how to parse it)
   * 
   * //! placeholder: return that query isn't updatable. return false; }
   * 
   * return true; }
   */
  ...
  private SQLElement seekTableFromColumn(SQLElement column) {
    ...
    Diagnostic.fail();
    return new SQLElement(column.getPrefixName(), null,
        column.getPrefix2Name(), column.getPrefix3Name(), null);
  }
  ...
  private boolean analyzeBestRowId(String databaseName, String tableName,
      String schemaName, SQLElement table) {
    // Ask the JDBC driver for the best row id for this table.
    boolean found = false;
    try {
      ResultSet resultSet = database.getMetaData().getBestRowIdentifier(
          databaseName, schemaName, tableName,
          database.getMetaData().bestRowTransaction, true);
    ...
    if (!found
        && bestRowIdForTable.size() == 0
        && database.getRuntimeMetaData().getBooleanValue(
            RuntimeMetaData.USE_ORACLEROWID)) {
      bestRowIdForTable.removeAllElements();
  ...
  private boolean analyzePrimaryKey(String databaseName, String tableName,
      String schemaName, SQLElement table) {
    boolean storeBestRow = (bestRowIdForTable.size() == 0);
    // Get primary key information.
    try {
      ResultSet resultSet = database.getMetaData().getPrimaryKeys(databaseName,
          schemaName, tableName);
      return findRowIdInResultSet(resultSet, table, false);
    } catch (Exception se) {
      // Some drivers (ie. Oracle's 7.3 driver) throw an exception instead of
      // failing quietly.
      validRowIdForTable.removeAllElements();
      if (storeBestRow)
        bestRowIdForTable.removeAllElements();
  ...
  private boolean findRowIdInResultSet(ResultSet resultSet, SQLElement table,
      boolean bestRow) throws SQLException {
    ...
    while (resultSet.next()) {
      ...
      // (note: right trimming string in case some driver returns meta-info as
      // CHAR not VARCHAR)
      String columnName = trimRight(resultSet.getString(ordinal));
      // !BUGTEST columnName = JdbcProvider.trimRight(columnName);
      Diagnostic.trace(Trace.QueryAnalyze,
          "(rowId)getBestRowIdentifier table,column: " + table.toString()
              + ", " + columnName);
      ...
      boolean quotes = !bestRow ? true
          : resultSet.getShort(8) != java.sql.DatabaseMetaData.bestRowPseudo;
  ...
  private boolean analyzeUniqueIndex(String databaseName, String tableName,
      String schemaName, SQLElement table) {
    
    // Get unique index information.
    ResultSet resultSet;
    try {
      resultSet = database.getMetaData().getIndexInfo(databaseName, schemaName,
          tableName, true, true);
    } catch (Exception ex) {
      Diagnostic.printStackTrace(ex);
      return false;
    ...
    try {
      while (resultSet.next()) {
        ...
        Diagnostic.trace(Trace.QueryAnalyze, "Added table,column:" + tableName
            + ", " + columnName + " to Unique key: " + indexName);
  ...
  private boolean analyzeUniqueIndex(Vector indexColumns, boolean storeBestRowId) {
    ...
    boolean storeBestRow = (bestRowIdForTable.size() == 0);
    if (storeBestRowId) {
      if (bestRowIdForTable.size() == 0
          || bestRowIdForTable.size() > indexColumns.size())
  ...
  int[] getOrdinalsOfRowIds() {
    ...
    for (int index = 0; index < nValid; index++)
      result[index + nNew] = nNew
          + columnList.indexOf(validRowId.elementAt(index));
    return result;
  }
  ...
  public static String removeHiddenColumns(QueryDataSet qds,
    ...
    SimpleParser parser = new SimpleParser(original, db
    ...
    for (String name : columns) {
      ...
      while (tokens != null) {
        if (tokens.getType() == SQLToken.FIELD
            && tokens.getLabelName().equalsIgnoreCase(name)) {
  ...
  private Vector bestRowIdForTable; // Potential row id column name list for a
  // single table.
  private Vector bestRowId; // Potential row id column name list for all tables
  // in the query.
  private Vector validRowIdForTable; // Active row id column name list for a
  // table.
  private Vector validRowId; // Active row id column name list for all tables in
  // the query.
  ...
  private Vector columnList; // Vector of type SQLElement of the columns in the
  // query.
  private Vector tableList; // Vector of type SQLElement of the tables in the
  // query
