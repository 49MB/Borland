...
class QueryParseToken extends SQLElement {
  ...
  QueryParseToken(int type, String name, String prefix1, String prefix2,
      String prefix3, String alias) {
  ...
  boolean isSelect() {
    return tokenType == SQLToken.SELECT;
  }
  
  boolean isExpression() {
    return tokenType == SQLToken.EXPRESSION || tokenType == SQLToken.CONSTANT
        || tokenType == SQLToken.FUNCTION;
  }
  
  boolean isField() {
    return tokenType == SQLToken.FIELD;
  }
  
  boolean isFunction() {
    return tokenType == SQLToken.FUNCTION;
  }
  
  boolean isParameter() {
    return tokenType == SQLToken.PARAMETER;
  }
  
  boolean isTable() {
    return tokenType == SQLToken.TABLE;
  }
  
  boolean isWhere() {
    return tokenType == SQLToken.WHERE;
  }
  
  // !NIY
  // Not determined yet whether we need to parse sub-queries. If not,
  // there should be no need to store an expression as a token.
  void setExpression(QueryParseToken token) {
    expression = token;
  }
  ...
  void setNextToken(QueryParseToken token) {
    nextToken = token;
  }
  
  QueryParseToken getNextToken() {
    return nextToken;
  }
