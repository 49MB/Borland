...
class CallableStmt extends PreparedStmt {
  ...
  void resetState(Database database, String query,
      MasterLinkDescriptor masterDetail, ReadWriteRow parameterRow,
      boolean[] inputOnly) /*-throws DataSetException-*/throws SQLException {
  ...
  // ! Warning: Overloaded method from PreparedStmt
  // ! Warning: This method is overloaded in InterbaseStmt and
  // OracleCallableStmt
  // !
  ...
  void setOutputValues() /*-throws DataSetException-*/throws SQLException {
    ...
    Diagnostic.trace(Trace.QueryProgress,
        "Procedure: getting OUT parameter values");
  ...
  boolean analyzeParameters(QueryParseToken tokens, int actualParamCount,
    ...
    boolean namedParameter = super.analyzeParameters(tokens, actualParamCount,
        quoteChar);
    ...
    for (int i = 0; i < columnCount; i++) {
      ...
      else if (readOnly != null && paramNo < readOnly.length
          && readOnly[paramNo])
        inputOverride[i] = true;
    }
  ...
  void bindParameter(Variant data, Column column, int param)
    ...
    int type = isInputOverride(param) ? ParameterType.IN : column
        .getParameterType();
    ...
    case ParameterType.IN_OUT:
      registerOutParameter(data, column, param); // Must be first because of BDE
                                                 // bug: 8860
      super.bindParameter(data, column, param);
      break;
  ...
  boolean bindOutParameter(Variant data, Column column, int param)
    ...
    case ParameterType.IN_OUT:
      ...
      case Variant.BIGDECIMAL:
        BigDecimal tempBigDecimal = statement.getBigDecimal(param, column
            .getScale());
  ...
  private boolean[] inputOverride; // Which parameters are input only, (given in
                                   // resetState)
  CallableStatement statement; // Instance of CallableStatement, (copy of
                               // PreparedStatement instance in
                               // PreparedStmt.java)
}
