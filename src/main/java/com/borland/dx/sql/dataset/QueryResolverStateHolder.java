...
public class QueryResolverStateHolder extends StateHolder {
  ...
  {
    Diagnostic.trace(Trace.DataSetSave, "QueryResolver inserting row "
        + dataSet.getRow());
    
    // If we are processing a different DataSet than when we were last invoked,
    // reset the
    ...
    for (int tableNo = 0; tableNo < cachedTableCount; ++tableNo) {
      ...
      try {
        ...
        insertQuery.setDataSet(storageDataSet, cachedTableCount,
            cachedColumnCount, tableNo);
        ...
        Diagnostic.trace(Trace.DataSetSave, "Insert values:  " + values.length
  ...
  {
    Diagnostic.trace(Trace.DataSetSave, "QueryResolver updating row: "
        + dataSet.getRow());
    ...
    for (int tableNo = 0; tableNo < cachedTableCount; ++tableNo) {
      ...
      try {
        ...
        // A new dataset will cause the query to be rebuilt.
        updateQuery.setDataSet(storageDataSet, cachedTableCount,
            cachedColumnCount, tableNo);
        
        Diagnostic.trace(Trace.DataSetSave,
            "QueryResolver looking for changes:");
        ...
        for (int index = 0; index < values.length; ++index) {
          ...
          // If updateWhereAll or this is a rowId, set rowId status for this
          // column.
          if (cachedColumns[index].isRowId()
              || ((resolver.getUpdateMode() == UpdateMode.ALL_COLUMNS)
                  && cachedColumns[index].isSearchable() && resolvable)) {
          ...
          Diagnostic.trace(Trace.DataSetSave, "Testing column: "
              + cachedColumns[index].getColumnName());
          
          if (resolvable && !values[index].equals(oldValues[index])) {
            // The old value and new don't match.
            noChanges = false;
            Diagnostic.trace(Trace.DataSetSave, "Testing column: "
                + cachedColumns[index].getColumnName() + " Changed!!!");
            ...
            // If updateWhereChanged and this isn't already a rowId, add this as
            // a rowId.
        ...
        Diagnostic.trace(Trace.DataSetSave, "QueryResolver no changes, count: "
            + noChanges + " " + count);
        
        // If this is a "phantom" change row, quit now. A phantom change row
        // means that all of the
        // values in the original and "new" row are the same (ie. no changes
        // where made).
        if (noChanges)
          continue;
        ...
        Diagnostic.trace(Trace.DataSetSave, "Update query value length:  "
            + values.length + " queryString:\n" + updateQuery.getQueryString());
  ...
  /*-throws DataSetException-*/
  {
    ...
    DataSetException.noRowsAffected(Res.bundle.format(ResIndex.NoRowsAffected,
        queryString, buffer.toString()));
  }
  
  void deleteRow(DataSet dataSet) /*-throws DataSetException-*/
  {
    Diagnostic.trace(Trace.DataSetSave, "QueryResolver deleting row "
        + dataSet.getRow());
    ...
    for (int tableNo = cachedTableCount - 1; tableNo >= 0; tableNo--) {
      ...
      try {
        ...
        // A new dataset will cause the query to be rebuilt.
        deleteQuery.setDataSet(storageDataSet, cachedTableCount,
            cachedColumnCount, tableNo);
        ...
        for (int index = 0; index < values.length; ++index) {
          ...
          if (cachedColumns[index].isRowId()
              || ((resolver.getUpdateMode() == UpdateMode.ALL_COLUMNS)
                  && cachedColumns[index].isSearchable() && cachedColumns[index]
                  .isResolvable())) {
            status |= ResolverQuery.rowId;
        ...
        Diagnostic
            .trace(Trace.DataSetSave, "delete query values length:  "
                + values.length + " delete query:\n"
  ...
  private void cacheDataSet() /*-throws DataSetException-*/{
    // WARNING. DataStore implementation of MatrixData uses separate
    // StorageDataSet
    ...
    if (age != structureAge || cachedColumns == null) {
      ...
      if (storageDataSet.getTableName() != null) {
        ...
        String tableName = db.makeTableIdentifier(null, storageDataSet
            .getSchemaName(), storageDataSet.getTableName());
        cachedTableCount = 1;
        ...
        for (int i = 0; i < cachedTableCount; i++) {
          if (tableHasRowId[i] != HAS_A_ROWID) {
            if (cachedTables == storageDataSet.getResolveOrder()) {
              ...
              System.arraycopy(storageDataSet.getResolveOrder(), 0,
                  cachedTables, 0, cachedTables.length);
            }
            cachedTables[i] = null;
          }
        }
  ...
  private final StorageDataSet storageDataSet;
  ...
  private static boolean raiseNoRowsException = true;
  
  public static boolean isRaiseNoRowsException() {
    return raiseNoRowsException;
  }
  
  public static void setRaiseNoRowsException(boolean raiseNoRowsException) {
    QueryResolverStateHolder.raiseNoRowsException = raiseNoRowsException;
  }
}
