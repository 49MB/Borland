...
public abstract class ResolutionManager {
  ...
  // Save a collection of DataSets within a Database. The method is synchronized
  // as the Resolver
  ...
  /*-throws DataSetException-*/
  {
    ...
    if (errorDataSets != null)
      throw new ResolutionException(ResolutionException.RESOLVE_PARTIAL,
          errorDataSets, Res.bundle.getString(ResIndex.ResolveFailed));
   ...
   * resolutionCalled = 0; StorageDataSet dataSet;
   * 
   * for (int index = 0; index < dataSets.length; ++index) {
   * dataSets[index].open(); dataSet = dataSets[index].getStorageDataSet(); for
   * (int index2 = 0; index2 < index; ++index) { if
   * (dataSets[index].getStorageDataSet() == dataSet) { dataSet = null;
   * dataSets[index] = null; break; } } if (dataSet != null) {
   * ProviderHelp.startResolution(dataSets[index].getStorageDataSet(),
   * postEdits); resolutionCalled++; } }
   * 
   * if (resolutionCalled < dataSets.length) { DataSet[] newDataSets = new
   * DataSet[resolutionCalled]; int destIndex = 0; for (int index = 0; index <
   * dataSets.length; ++index) { if (dataSets[index] != null) {
   * newDataSets[destIndex] = dataSets[index]; ++destIndex; } } return
   * newDataSets; }
   * 
   * return dataSets; }
   */
  ...
  /*-throws DataSetException-*/
  {
    for (int index = resOrder.length - 1; index >= 0; index--) {
      ...
      if (sds.getUpdatedRowCount() > 0) {
        ...
        ResolutionResolver resolver = (ResolutionResolver) defaultResolver
            .getResolver(dataSet);
        try {
    ...
    for (DataSet dataSet : resOrder) {
      ...
      if (saveAll || sds.getInsertedRowCount() > 0) {
        ...
        ResolutionResolver resolver = (ResolutionResolver) defaultResolver
            .getResolver(dataSet);
        try {
          processInserts(resolver, saveAll ? (DataSet) sds : insertDataSet,
              dataSet);
        } finally {
          resolver.close(sds);
          insertDataSet.close();
  ...
  private void processInserts(ResolutionResolver resolver,
  ...
  private void processDeletes(ResolutionResolver resolver,
      DataSet deleteDataSet, DataSet dataSet)
  /*-throws DataSetException-*/
  {
  ...
  private void processUpdates(ResolutionResolver resolver,
  ...
  private void processInsertRow(ResolutionResolver resolver,
      DataSet insertDataSet, DataSet dataSet)
  /*-throws DataSetException-*/
  {
    // Event handling code is here instead of the resolver so that each
    // implementation
    ...
    if (resolverListener == null) {
      try {
        ...
        logError(ResolutionException.INSERT_FAILED, dataSet, insertDataSet, ex,
            true, false);
      }
    } else {
      ...
      while (true) {
        ...
        try {
          ...
          if (response.isRetry())
            ...
            logError(ResolutionException.INSERT_FAILED, dataSet, insertDataSet,
                ex, false, false);
            return;
          }
        }
  ...
  private void processDeleteRow(ResolutionResolver resolver,
      DataSet deleteDataSet, DataSet dataSet)
  /*-throws DataSetException-*/
  {
    // Event handling code is here instead of the resolver so that each
    ...
    if (resolverListener == null) {
      try {
        ...
        logError(ResolutionException.DELETE_FAILED, dataSet, deleteDataSet, ex,
            true, false);
      }
    } else {
      DataSetException abortEx = null;
      while (true) {
        ...
        try {
          ...
          if (response.isRetry())
            ...
            logError(ResolutionException.DELETE_FAILED, dataSet, deleteDataSet,
                ex, false, false);
            return;
          }
        }
  ...
  private void processUpdateRow(ResolutionResolver resolver,
      DataSet updateDataSet, ReadWriteRow oldRow, DataSet dataSet)
  /*-throws DataSetException-*/
  {
    // Event handling code is here instead of the resolver so that each
    // implementation
    ...
    if (resolverListener == null) {
      try {
        ...
        Diagnostic.println("dataSet.getRow:  " + dataSet.getRow());
        ResolutionException.updateFailed(dataSet, null, ex);
        logError(ResolutionException.UPDATE_FAILED, dataSet, updateDataSet, ex,
            true, false);
      }
    } else {
      ...
      while (true) {
        ...
        try {
          resolverResponse.resolve();
          resolverListener.updatingRow(updateDataSet, updateRow,
              resolverResponse);
          if (resolverResponse.isSkip())
            return;
          if (resolverResponse.isAbort())
          ...
          if (newData == null) {
            // !JOAL Quick Hack:
            // We create a virtual original row here, which allows the user to
            // change
          ...
          resolverListener.updateError(dataSet, updateDataSet, oldRow,
              updateRow, ex, response);
          if (response.isRetry())
            continue;
          else if (response.isAbort()) {
            abort = true;
            ...
            logError(ResolutionException.UPDATE_FAILED, dataSet, updateDataSet,
                ex, false, false);
            return;
          }
        }
  ...
  // This property can be used to change the default order of actions for the
  // resolution manager.
  // By default, we will resolve deletes, updates, then inserts. In some corner
  // cases (specifically,
  // for master/detail DataSets where link fields are changed), it may be
  // desirable to process
  // inserts before updates (ie. insert a new master row, change existing detail
  // rows to point to
  // the new master row). Default behavior (as stated above) is to process
  ...
  /**
   * @since JB2.0 This property controls whether edits should be posted before
   *        resolution starts. If this property is set to false, the changes of
   *        the current row is not posted and the changes are not resolved back
   *        to the data source. Default is true.
   */
  ...
  /**
   * @since JB2.0 This property controls whether the status of the datasets
   *        involved should be reset after the resolution is done. If this
   *        property is set to false, the status bits has to be reset by calling
   *        resetPendingStatus for an array of dataSets, or calling
   *        resetPendingStatus directly on the dataSet. Default is true.
   */
  ...
  protected abstract void initError(int code, DataSet dataSet, DataSet view,
      ResolveError resolveError);
  
  final void logError(int code, DataSet dataSet, DataSet view, Exception ex,
      boolean failIfCantLog, boolean fail)
  /*-throws DataSetException-*/
  {
    ...
    if (errorDataSet != null) {
      ...
      error.response = fail ? ErrorResponse.ABORT : ErrorResponse.IGNORE;
      if (ex != null) {
      ...
      if (errorCounts[index] > maxErrors && maxErrors > -1)
        throw new ResolutionException(ResolutionException.RESOLVE_FAILED,
            errorDataSets, Res.bundle.getString(ResIndex.ResolveFailed));
    }
  ...
  private final ResolverResponse resolverResponse;
