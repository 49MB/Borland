...
class QueryAnalyzer {
  ...
  Vector getTables() /*-throws DataSetException-*/{
    ...
    while (tokens != null) {
      ...
      if (tokens.isTable()) {
        // If the tableName is not quoted, see if we need to upper or lower case
        // the string in
        // the query. The rationale behind this is that we use the tableName in
        // calls to the
        // Jdbc API. The Jdbc API treats literals (unquoted strings) as quoted
        // strings when
        // passing the string to the server. Therefore, a query like:
        // "select * from foo" against
        // an Oracle server will work if the relation is stored as "FOO" on the
        // server. When we
        // pass the string "foo" to Jdbc to get column, index, ... information,
        // Jdbc asks the
  ...
  Vector getColumns() /*-throws DataSetException-*/{
    ...
    while (tokens != null && !tokens.isWhere()) {
      ...
      if (tokens.isField()) {
        ...
        element.unquoteStrings(this);
        result.addElement(element);
      } else if (tokens.isExpression() && tokens.fixAlias()) {
        SQLElement element = new SQLElement(tokens);
        element.unquoteStrings(this);
        result.addElement(element);
  ...
  String unquoteString(String dbString) {
    ...
    if (dbString.charAt(0) != quoteCharacter) {
      ...
      // Store the string without quotes (JDBC API doesn't require quotes for
      // parameters).
  ...
  void init() /*-throws DataSetException-*/{
    ...
    if (quoteCharacter != '\0') {
      ...
      mkUpperQuoted = isCaseInsensitiveQuoted
          && !database.storesLowerCaseQuotedIdentifiers();
  ...
  private final String query; // setup in constructor
