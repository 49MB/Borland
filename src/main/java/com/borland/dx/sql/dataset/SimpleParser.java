...
class SimpleParser extends SimpleScanner implements SQLToken {
   ...
   * "select scott.sampleemp.lname, ipgdev.sampleemp.lname from scott.sampleemp, ipgdev.sampleemp where scott.sampleemp.empid = ipgdev.sampleemp.empid"
   * );test(
   * "select A.NAME, B.STREET, B.NAME as Place from M_PERSON A, M_ADDRESS B where A.ID=B.ID"
   * );test(
   * "select distinct col1,?,:par,fct(2,:hello,9) from table1 where y = :param and x = :1.param order by :param"
   * );test(
   * "select col1,col2,col3+45,db.schema.table1.colx,avg(col) from table1,table2 where x = 3.5 or g <= u"
   * ); test("select \"SELECT\" from \"DATE\"");
   * test("select \"\"\"SE\"\"LECT\" from \"DATE\"\"\""); } static void
   * test(String query) {System.out.println(
   * "---------------------------------------------------------------------------------"
   * ); System.out.println("Original Query:\n   "+query); SimpleParser parser =
   * new SimpleParser(query,'"'); QueryParseToken tokens; tokens =
   * parser.getParameterTokens();
   * System.out.println("Generated from Parameter Parse ("
   * +parser.getParameterCount()+" parameters):\n   "+parser.format(tokens));
   * tokens = parser.getParsedTokens();
   * System.out.println("Generated from Fully Parse ("
   * +parser.getParameterCount()+" parameters):\n   "+parser.format(tokens)); }
   */
  ...
  private boolean charOk(char c) {
    return Character.isJavaIdentifierPart(c) || c == '\'';
  }
  
  private StringBuffer appendSpaceWhenChar(StringBuffer s, String name) {
    if (name.length() == 0)
      return s;
    if (s.length() > 0) {
      char c = s.charAt(s.length() - 1);
      if (charOk(c) && charOk(name.charAt(0)))
        s.append(" ");
    }
    s.append(name);
    return s;
  ...
  private boolean isJoinType(String name, boolean isTable) {
    return (isTable && name.equalsIgnoreCase("ON"))
        || name.equalsIgnoreCase("INNER") || name.equalsIgnoreCase("OUTER")
        || name.equalsIgnoreCase("LEFT") || name.equalsIgnoreCase("RIGHT")
        || name.equalsIgnoreCase("FULL");
  }
  ...
  private void select_stmt() {
    ...
    // (SS) Firebird: SELECT FIRST X ...:
    if (symbol == SCAN_FIRST) {
      addToken(new QueryParseToken(SQLToken.OTHER, getCurrentID()));
      symbol = nextToken();
    ...
    while (symbol == SCAN_ID) {
      StringBuffer joinsql = new StringBuffer();
      name = getCurrentID();
      if (isJoinType(name, false)) {
        joinsql.append(name);
        symbol = nextToken();
        name = getCurrentID();
      }
      if (name.equalsIgnoreCase("JOIN")) {
        appendSpaceWhenChar(joinsql, name);
        symbol = nextToken();
        
        if (hasStar || (symbol != SCAN_ID && symbol != SCAN_LEFT_PAREN))
          throw new RuntimeException(); // We cannot parse this...
        ...
        do {
          ...
          case SCAN_RIGHT_PAREN:
            parentesis--;
            if (parentesis < 0)
              throw new RuntimeException("SQL Unparseable");
            break;
          case SCAN_WHERE:
          case SCAN_GROUP:
          case SCAN_HAVING:
          ...
          case SCAN_EOQ:
            ...
            break;
          case SCAN_ID:
            if (isJoinType(name, false) || name.equalsIgnoreCase("JOIN")
                && parentesis == 0)
              end = true;
            break;
          }
        ...
        } while (!end && symbol != SCAN_EOQ);
        addToken(new QueryParseToken(SQLToken.JOIN, joinsql.toString()));
      } else
        break;
    }
  ...
  private void select_item() {
    ...
    if (!isExpression)
      addToken(new QueryParseToken(SQLToken.FIELD, name, prefix1, prefix2,
          prefix3, alias));
    else
      addExpressionToken();
  }
  ...
  private void table_ref() {
    ...
    if (symbol == SCAN_ID) {
      alias = getCurrentID();
      if (isJoinType(alias, true) || alias.equalsIgnoreCase("JOIN"))
        alias = null;
      else
        symbol = nextToken();
    }
    addToken(new QueryParseToken(SQLToken.TABLE, name, prefix1, prefix2, null,
        alias));
  ...
  private void scalar_expr() {
    scalar_term();
    while (symbol == SCAN_PLUS_MINUS || symbol == SCAN_STAR
        || symbol == SCAN_OTHER_OPERATOR) {
      isExpression = true;
      symbol = nextToken();
      scalar_term();
    }
  ...
  private void garbage_with_parens() {
    Diagnostic.check(symbol == SCAN_LEFT_PAREN);
    int parenLevel = 1;
    symbol = nextToken();
  ...
  // column_ref__or__function_ref = [ prefix3 "." [ prefix2 "." [ prefix1 "."
  // ]]] name [ "(" garbage_with_parens ")" ]
  ...
  private QueryParseToken parseFoundParameter() {
    ...
    Diagnostic.check(symbol == SCAN_COLON);
    String name = null;
    symbol = nextToken();
    if (symbol == SCAN_NUMBER) {
      name = getCurrentID();
...
class SimpleScanner {
  ...
  int nextToken() {
    ...
    while (pos <= end) {
      ...
      case ';':
        return SCAN_EOQ;
      case '(':
        return SCAN_LEFT_PAREN;
      case ')':
        return SCAN_RIGHT_PAREN;
      case '+':
        return SCAN_PLUS_MINUS;
      case '~':
        return SCAN_PLUS_MINUS; // Sybase bitwise NOT operator
      case '%':
        return SCAN_OTHER_OPERATOR; // Sybase modulo operator
      case '&':
        return SCAN_OTHER_OPERATOR; // Sybase bitwise AND operator
      case '^':
        return SCAN_OTHER_OPERATOR; // Sybase bitwise EXCLUSIZE OR operator
      case '*':
        hasStar = true;
        return SCAN_STAR;
      case '/':
      ...
      case '.':
        if (selectQuery[pos] < '0' || selectQuery[pos] > '9')
          return SCAN_DOT;
        start = --pos;
        skipNumber();
        return SCAN_NUMBER;
      default:
        ...
        if (ch >= '0' && ch <= '9') {
          ...
          return SCAN_NUMBER;
            ...
            ch >= 'A' && ch <= 'Z' || Character.isLetter(ch)) {
          start = pos - 1;
          return skipID();
        } else {
          return SCAN_ERROR;
  ...
  private void skipUntilEndComment() {
    int stop = end - 1;
    while (pos <= stop
        && !(selectQuery[pos] == '*' && selectQuery[pos + 1] == '/'))
      pos++;
    pos += 2;
  }
  ...
  private int lookupID() {
    switch (pos - start) {
    case 17:
      if (match(SYM_CURRENT_TIMESTAMP))
        return SCAN_CURRENT_TIMESTAMP;
      break;
    case 12:
      if (match(SYM_CURRENT_DATE))
        return SCAN_CURRENT_DATE;
      if (match(SYM_CURRENT_TIME))
        return SCAN_CURRENT_TIME;
      break;
    case 9:
      if (match(SYM_TIMESTAMP))
        return SCAN_TIMESTAMP;
      break;
    case 8:
      if (match(SYM_DISTINCT))
        return SCAN_DISTINCT;
      break;
    case 6:
      if (match(SYM_SELECT))
        return SCAN_SELECT;
  ...
  final static int SCAN_EQUALS = 29;
  
  final static int SCAN_FIRST = 30;
  ...
  final static String SYM_FIRST = "FIRST"; // (SS) Firbird limit funktion
  
  // Members:
  private final char[] selectQuery;
  private int mark;
  private int start;
  private int pos;
  private final int end;
  private int prevPos;
  private final boolean idQuote;
