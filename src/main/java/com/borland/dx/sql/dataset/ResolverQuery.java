...
class ResolverQuery {
  ...
  final void setDataSet(DataSet dataSet, int tables, int columnCounts[],
      int newTable) {
    if (this.dataSet != dataSet || this.tables != tables
        || !columnCounts.equals(this.columnCounts)) {
  ...
  final int execute() throws SQLException {
    Diagnostic.trace(Trace.DataSetSave, "Executing query: " + query);
    return preparedStatement.executeUpdate();
  }
  
  protected final void prepare(String query) throws SQLException,
      DataSetException {
    // // If we statement caching is possible, then close the current statement
    // only,
    // // otherwise close all other open statement.
    // if (database.isUseStatementCaching() && database.getMaxStatements() >
    // tables+2)
    
    close(currentTable);
    Diagnostic.trace(Trace.DataSetSave, "Preparing query: " + query);
    ...
    Diagnostic.trace(Trace.DataSetSave, "Query prepared:");
    this.query = query;
  }
  ...
  protected final void setParameter(int index, Column column, Variant value)
    ...
    Diagnostic.check(preparedStatement != null);
    Diagnostic.trace(Trace.DataSetSave, "setParameter " + index + " column " + // NORES
    ...
    case Variant.STRING:
      ...
      if (!useObjectForStrings) {
        ...
        preparedStatement.setObject(index, stringValue,
            java.sql.Types.LONGVARCHAR, 0);
      } else if (sqlType1 == java.sql.Types.VARCHAR || sqlType1 == 0) {
        preparedStatement.setObject(index, stringValue, java.sql.Types.VARCHAR,
            0);
      } else {
        
        // If precision is set, and the string isn't long enough, append spaces
        // to pad the
        // string, otherwise use default behavior.
        //
        // Space padding is needed for some JDBC drivers when trying to bind a
        // VARCHAR parameter
        // for comparison with a CHAR field in a relation (ie. where clause
        // parameter is a
        ...
        if (useSpacePadding) {
          ...
          if ((precision != -1) && (length < precision)) {
            ...
            preparedStatement.setObject(index, buf.toString(),
                java.sql.Types.CHAR, 0);
          } else
            preparedStatement.setString(index, stringValue);
        } else {
          preparedStatement.setString(index, stringValue);
    ...
    case Variant.INPUTSTREAM:
      ...
      if (stream == null)
        ...
        try {
          ...
          // will reset to the start if a mark was never set.
          // BufferedInputStream
          // will throw an exception if a mark was never set to hold the
          // complete InputStream.
          //
          stream.reset();
    ...
    case Variant.DATE:
      preparedStatement.setDate(index, value.getDate());
      break;
  ...
  protected final void columnString(Column column, FastStringBuffer buf) {
    ...
    if (quoteCharacter != '\0') {
      ...
      // a way to fool the parser into accepting these columns.
      {
        ...
        // ! JOAL: Weirdness, we want a reference to a pseudo column INTERNALROW
        // to appear without quotes
        // ! But pseudo property is not available: use rowId && !resolvable &&
        // isInternalRow
        boolean skipColumnQuotes = (column.isRowId() && !column.isResolvable() && result
            .equals("INTERNALROW")); // NORES
        if (!skipColumnQuotes)
  ...
  protected final void whereClause(FastStringBuffer buf) {
    ...
    for (int index = 0; index < currentColumn; index++) {
      ...
      if ((status & rowId) != 0) {
        ...
        Diagnostic.check(column.isSearchable() ? null : "column: "
            + column.getColumnName() + " shouldn't be in WHERE clause!");
  ...
  /*-throws DataSetException-*/throws SQLException {
    Diagnostic.trace(Trace.DataSetSave, "Setting Where parameters");
    for (int index = 0; index < currentColumn; index++) {
      int status = columnStatus[index];
      if ((status & rowId) != 0)
        if ((status & nullRowId) == 0) {
          ...
          Diagnostic.check(column.isSearchable(), "column: "
              + column.getColumnName() + "shouldn't be bound in WHERE clause!");
  ...
  private final boolean useObjectForStrings;
  private final boolean useSpacePadding;
