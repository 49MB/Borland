...
public class SQLElement {
  ...
  SQLElement(String name, String alias, String prefix1, String prefix2,
      String prefix3) {
    tokenName = name;
    tokenAlias = alias;
  ...
  public boolean equals(Object object) {
    ...
    return equals(tokenName, other.tokenName)
        && (!tokenName.equals("INTERNALROW") || (quoted & 1) == (other.quoted & 1))
        && // Special for JDataStore quote is meaningfull for INTERNALROW
  ...
  public String toString() {
    return "{" + tokenName + ", " + tokenPrefix3 + ", " + tokenPrefix2 + ", "
        + tokenPrefix1 + ", " + tokenAlias + "}";
  }
  
  // General stuff
  
  boolean equalElement(SQLElement anElement) {
    return (anElement != null) && equals(tokenName, anElement.tokenName)
  ...
  void mkColumnOfTable(SQLElement table, boolean quoteName) {
    if (table.tokenAlias != null) {
      ...
      quoted = (quoteName ? 1 : 0) + ((table.quoted & 1) != 0 ? 4 : 0)
          + ((table.quoted & (4 + 8)) << 1);
    }
  }
  
  boolean isColumnFromTable(SQLElement table, String userName) {
    Diagnostic.check(tokenPrefix1 != null);
    if (table.tokenAlias != null)
      return (tokenPrefix1.equalsIgnoreCase(table.tokenAlias)
          && tokenPrefix2 == null && tokenPrefix3 == null);
    else {
      if (!tokenPrefix1.equalsIgnoreCase(table.tokenName))
        return false;
      if (tokenPrefix2 == null && table.tokenPrefix1 != null
          && !table.tokenPrefix1.equalsIgnoreCase(userName))
        return false;
      if (tokenPrefix2 != null
          && (table.tokenPrefix1 != null ? !tokenPrefix2
              .equalsIgnoreCase(table.tokenPrefix1) : !tokenPrefix2
              .equalsIgnoreCase(userName)))
        return false;
      return (tokenPrefix3 == null ? table.tokenPrefix2 == null : tokenPrefix3
          .equalsIgnoreCase(table.tokenPrefix2));
    }
  }
  ...
  public String getLabelName() {
    return tokenAlias != null ? tokenAlias : tokenName;
  }
  ...
  void setQuotes(boolean nameQuote, boolean aliasQuote, boolean prefix1Quote,
      boolean prefix2Quote, boolean prefix3Quote) {
    quoted = 0;
    if (nameQuote)
      quoted += 1;
    if (aliasQuote)
  ...
  void unquoteStrings(QueryAnalyzer analyzer) {
    quoted |= tokenName != null
        && tokenName.charAt(0) == analyzer.quoteCharacter ? 1 : 0;
    quoted |= tokenAlias != null
        && tokenAlias.charAt(0) == analyzer.quoteCharacter ? 2 : 0;
    quoted |= tokenPrefix1 != null
        && tokenPrefix1.charAt(0) == analyzer.quoteCharacter ? 4 : 0;
    quoted |= tokenPrefix2 != null
        && tokenPrefix2.charAt(0) == analyzer.quoteCharacter ? 8 : 0;
    quoted |= tokenPrefix3 != null
        && tokenPrefix3.charAt(0) == analyzer.quoteCharacter ? 16 : 0;
  ...
  private void appendItem(StringBuffer buffer, String item, int mask,
      char quoteCharacter, boolean dot) {
    if ((quoted & mask) != 0)
  ...
  String generateString(char quoteCharacter, boolean ignorePrefix,
      boolean ignoreAlias) {
    ...
    // Add Prefix:
    if ((!ignorePrefix || tokenName.equals("*") || tokenPrefix2 != null)
        && tokenPrefix1 != null) {
      if (tokenPrefix3 != null)
  ...
  public boolean fixAlias() {
    if (tokenAlias != null)
      return true;
    
    if (tokenName != null) {
      String[] s = tokenName.split("\\sAS\\s", 2);
      if (s.length == 2) {
        tokenName = s[0];
        tokenAlias = s[1];
        return true;
  ...
  private String tokenPrefix1; // Table or Schema: table1.column1 or
  // schema1.table1
  private String tokenPrefix2; // Schema or Database: schema1.table1.column1 or
  // db1.schema1.table1
  private String tokenPrefix3; // Database: db1.schema1.table1.column1
  private String tokenAlias; // The column/table alias: col1 as fname or table1
  // as t1
  private int quoted;
}
