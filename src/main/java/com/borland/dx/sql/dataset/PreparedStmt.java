...
public class PreparedStmt {
  ...
  public void resetState(Database database, String query,
      MasterLinkDescriptor masterDetail, ReadWriteRow parameterRow)
      /*-throws DataSetException-*/throws SQLException {
    ReadWriteRow[] rows = parameterRow == null ? null
        : new ReadWriteRow[] { parameterRow };
    resetState(database, query, masterDetail, rows);
  }
  
  public void resetState(Database database, String query,
      MasterLinkDescriptor masterDetail, ReadWriteRow[] parameterRows)
      /*-throws DataSetException-*/throws SQLException {
  ...
  /*-throws DataSetException-throws SQLException*/{
    try {
      ...
      Diagnostic.println(ex.getMessage());
    }
    
    // We need to be dumber about statement caching. We don't trust the return
    // from getMaxStatements anymore. JdbcOdbc bridge using interbase returns <
    // 1
    // and a SQL server jdbc drivers also returns something <= 10. So now the
    // developer must us a manual override
    ...
    closeStatement();
    /*
     * boolean doClose = true; try { doClose =
     * !(database.isUseStatementCaching() && database.getMaxStatements() > 10);
     * } finally { if (doClose) closeStatement(); }
     */
  }
  
  // ! Warning: This method is overloaded in CallableStmt
  public PreparedStatement createStatement(String queryString) /*-throws DataSetException-*/{
    PreparedStatement newStmt = statement;
    try {
      if (statement == null // || statement.isClosed()
          || !lastQuery.equals(queryString)) {
        newStmt = database.createPreparedStatement(queryString);
  ...
  void prepareParameters() /*-throws DataSetException-*/throws SQLException {
    ...
    if (paramColumns == null) {
      internalQuery = query;
      if (parameterRows != null || masterDetail != null) {
        ...
        // !kna boolean namedParameters =
        // analyzeParameters(tokens,parser.getParameterCount());
        boolean namedParameters = analyzeParameters(tokens,
            parser.getParameterCount(), quoteCharacter);// !kna
        if (namedParameters)
    ...
    // System.out.println("Fetching " + fetchSize + "/" + maxLoadRows);
  ...
  boolean analyzeParameters(QueryParseToken tokens, int actualParamCount,
      char quoteChar) /*-throws DataSetException-*/{// !kna
    parameterRowCount = 0;
    ...
    while (tokens != null) {
      if (tokens.isParameter()) {
        ...
        if (tokens.getName() == null) {
          ...
          if (namedParameters)
            ...
            if (actualParamCount > paramMaxCount)
              DataSetException.parameterCountMismatch(paramCount,
                  paramMaxCount, 0);
        ...
        // Else, the parameters are named, get parameter values from row by
        // parameter name.
        else {
          // Validate parameter format
    ...
    Diagnostic.trace(Trace.QueryProgress, "Setting columnCount");
    columnCount = paramCount;
    
    return namedParameters;
  }
  
  // !kna void classifyNamedParameter(String columnName, int param) /*-throws
  // DataSetException-*/ {
  private void classifyNamedParameter(String columnName, int param,
      char quoteChar) /*-throws DataSetException-*/{// !kna
    // !kna, for to fix Bug #19671...
    boolean quoted = (quoteChar != ' ' && columnName.length() >= 2
        && quoteChar == columnName.charAt(0) && quoteChar == columnName
        .charAt(columnName.length() - 1));
    if (quoted) {
    ...
    // Allow user to include parameterRow specification i.e. " {call
    // proc(:2.ColumnId)}
    // This would mean use parameterRows[1] i.e. number is 1 based (0 means
    // masterLink).
    //
    if (pos > 0) {
      ...
      paramNo--;
      if (paramNo < 0) {
        Column column = (masterDetail == null) ? null : masterDetail
            .getMasterDataSet().hasColumn(columnName);
        if (column != null) {
          String masterColumns[] = masterDetail.getMasterLinkColumns();
          for (String masterColumn : masterColumns) {
            if (columnName.equalsIgnoreCase(masterColumn)) {
  ...
  private void classifyOrdinalParameters(int paramCount) /*-throws DataSetException-*/{
    ...
    if (masterDetail != null) {
      String masterColumns[] = masterDetail.getMasterLinkColumns();
      if (parameterRows != null)
        System.arraycopy(masterColumns, 0, paramColumns, 0,
    ...
    if (parameterRows != null) {
      for (int i = 0; i < parameterRowCount; i++) {
        ReadWriteRow parameterRow = parameterRows[i];
        String parameterRowColumns[] = parameterRow.getColumnNames(paramCount
            - foundParams);
        if (masterDetail != null || parameterRowCount > 1)
          System.arraycopy(parameterRowColumns, 0, paramColumns, foundParams,
              parameterRowColumns.length);
        else
  ...
  public void setFetchAsNeededData(ReadRow fetchAsNeededData) {
    this.fetchAsNeededData = fetchAsNeededData;
  }
  ...
  void bindParameters() throws SQLException, DataSetException {
    ...
    if (fetchAsNeededData != null)
      master = fetchAsNeededData;
  ...
  void bindParameter(Variant data, Column column, int param)
    ...
    if (data.isNull()) {
      ...
      switch (data.getType()) {
      case Variant.STRING:
        Diagnostic.trace(Trace.QueryProgress, "Setting param: " + param
  ...
  public void setMaxRowCount(int maxLoadRows) {
    this.maxRowCount = maxLoadRows;
  }
  ...
  ReadRow fetchAsNeededData; // (SS) optimized fetching for
  // fetchAsNeeded-Masterlinks
}
