...
/**
 * ProcedureDataSet used to retrieve and edit data from JDBC Stored Procedure
 * data sources.
 *The <CODE>ProcedureDataSet</CODE> class is an extension of its superclass
*{@link com.borland.dx.dataset.StorageDataSet}<CODE>StorageDataSet</CODE></A>)
* and provides functionality
*to run a stored procedure against data stored in a SQL database,
*  passing in parameters if the procedure
*expects them. The procedure call is expected to return a cursor.
*
*<!--The procedure call is expected to return a cursor, or in the case of
* an InterBase select
*procedure, output that can be used to generate a <CODE>DataSet</CODE>.  -->
*
*<P>In any application that uses the <CODE>ProcedureDataSet</CODE>, the following components are also required:
*<UL>
*<LI>an instantiated {@link com.borland.dx.sql.dataset.Database}
* <CODE>Database</CODE></A> component to handle the JDBC connection to the SQL database
*<LI>an instantiated  {@link com.borland.dx.sql.dataset.ProcedureDescriptor}
*  <CODE>ProcedureDescriptor</CODE></A> object to store the stored procedure's
*  properties
*</UL>
*
*
*<P>
*The data contained in a <CODE>ProcedureDataSet</CODE> is the result of the most recent execution of the
*stored procedure. Storing the "result set" from the execution of the stored procedure in a
*
*<CODE>ProcedureDataSet</CODE> allows for greater flexibility in navigation of the resulting data.
*
*<P>The <CODE>ProcedureDataSet</CODE> inherits the
*{@link com.borland.dx.dataset.StorageDataSet} <CODE>maxRows</CODE></A>
* property which
*allows you to set the maximum number of rows stored initially in the <CODE>
* ProcedureDataSet</CODE> as a
*result of the stored procedure execution.
*
*<P>Once the data is stored in the <CODE>ProcedureDataSet</CODE>, you manipulate it and connect it to UI controls in exactly the same way as you would other <CODE>StorageDataSet</CODE> components, without regard to which component is storing the data.
*
*<P>This class is used with servers whose JDBC driver supports executing  stored procedures that generate a result set. Not all JDBC drivers support this; some vendor libraries require special API calls to invoke stored procedures. Refer to your server documentation for more information on whether it meets this requirement.
*
*<!-- JDS start - remove paragraph -->
*<P>For an example on executing stored procedures, see "Obtaining data through a stored procedure" in the <CITE>Database Application Developer's Guide</CITE>.
*<!-- JDS end -->
*
*
...
*
*<P><STRONG>Note: </STRONG>Stored "Functions" work with
*older versions of Oracle Servers as well.
*
*<P>The following example demonstrates a stored procedure in a package:
*
*<PRE><CODE>CREATE PACKAGE my_pack is
...
*  END sp_test;
*END my_pack;</CODE></PRE>
*
*
*
*<P>The call string for this procedure should be:
*<PRE><CODE>"{ call my_pack.sp_test(?) }"</CODE></PRE>
*No parameter row is needed.  The cursor is used to load the <CODE>ProcedureDataSet</CODE>.
*
*
*<P>The result set you need to load the data into the <CODE>ProcedureDataSet</CODE> with must be the first parameter
*in the stored procedure argument list.  If additional parameters need to be sent or
*received, specify the <CODE>ParameterRow</CODE> in the <CODE>Procedure</CODE>
*property (or programmatically through the <CODE>ProcedureDescriptor</CODE>).
*
*
*<A NAME="Sybase"></A>
...
*
*
*<P>Stored procedures on Sybase servers are created in a "Chained" transaction mode. In order to call Sybase stored procedures as part of a {@link com.borland.dx.sql.dataset.ProcedureResolver}<CODE>ProcedureResolver</CODE></A>, the procedures must be modified to run in an unchained transaction mode.  Use the Sybase stored system procedure <CODE>sp_procxmode</CODE> to change the transaction mode to either "anymode" or "unchained". See your Sybase documentation for additional information.
...
public class ProcedureDataSet extends StorageDataSet
{
   ...
   */
  public final ProcedureDescriptor getProcedure() { return procedureDescriptor; }

  /**
   *
   *
   */
  public final String getQueryString() { return procedureDescriptor != null ? procedureDescriptor.getQueryString() : null; }
 /**
  *
  */
  public final Database getDatabase()  { return procedureDescriptor != null ? procedureDescriptor.getDatabase() : null; }
  ...
  /**
   * Used internally to specify whether saveChanges() is always supported by the
   * ProcedureDataSet.
   *
   */
  ...
  /**
   * Saves changes made to the data. If no resolver has been specified,
   * a QueryResolver is used by default.
   * @param dataSet
   */
