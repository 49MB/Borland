...
public class DesignerConnectionCache {
  private static Vector<DesignerConnectionCache> cache = new Vector<DesignerConnectionCache>();
  ...
  public DesignerConnectionCache(ConnectionDescriptor cDesc,
      Connection connection) {
   ...
   */
  static synchronized void addConnection(ConnectionDescriptor cDesc,
      Connection connection) {
    
    // !Diagnostic.println("++ add new Connection: " + connection + ", desc = "
    // + cDesc);
  ...
  /**
   * Determines whether the given ConnectionDescriptor is already associated
   * with a Connection. If yes, bumps the use count up by 1 and returns
   * associated Connection. Returns null to show this ConnectionDescriptor is
   * not in use yet
   */
  public static synchronized Connection shareConnection(
      ConnectionDescriptor cDesc) {
    
    if (cDesc == null)
      return null;
    
    for (int i = 0; i < cache.size(); ++i) {
      DesignerConnectionCache cachedEntry = (DesignerConnectionCache) cache
          .elementAt(i);
      if (cachedEntry.connectionDescriptor.canShare(cDesc)) {
        ++cachedEntry.useCount;
        // ! cache.setElementAt(cachedEntry, i);
        // ! //!Diagnostic.println("    shareConnection: sharing " +
        // cachedEntry.connection + ", useCount at " + cachedEntry.useCount);
        // ! //dumpCache();
  ...
  /**
   * Called when done with a Connection, this method decrements the useCount and
   * frees the cache entry when it hits zero. It returns true iff there are no
   * more references to this connection.
   */
  static synchronized boolean releaseConnection(Connection connection) {
    for (int i = 0; i < cache.size(); ++i) {
      ...
      if (connection == cachedEntry.connection) {
        // !Diagnostic.println("    releaseConnection: useCount at " +
        // cachedEntry.useCount);
    ...
    // !Diagnostic.println(
    // "!! connection not cached -- signalling final release !!");
    return true;
  }
  
  // Should only be called on shutDown!!! Does not know about Database component
  // and
  // Database.component is tricky about how it really does closing and we are
  // about
  ...
  public synchronized static void shutDown() {
    for (int i = 0; i < cache.size(); ++i) {
      try {
        ...
        Diagnostic.fail(); // Oh well.
      }
    }
    // For insurance.
    //
    cache = new Vector<DesignerConnectionCache>();
  }
  ...
  // ! for (int i = 0; i < cache.size(); ++i) {
  // ! DesignerConnectionCache cachedEntry = (DesignerConnectionCache)
  // cache.elementAt(i);
  // ! Diagnostic.println("Cache entry[" + i + "] use count " +
  // cachedEntry.useCount + ", connection = " + cachedEntry.connection);
...
class Shutdown extends Thread {
  ...
  public final void run() {
    if (cache != null) {
      Diagnostic.println(toString() + " AUTOSHUTDOWN"); // NORES
      cache.shutDown();
    }
  }
}
