...
public class BorlandMetaData extends MetaDataImplementor {
  ...
  //
  // it is OK to use the remote class if a driver of the correct name exists,
  // and
  // it is indeed a java.sql.Driver. If not found or we are really looking at
  // the
  ...
  static public void getBorlandRemoteURLs(String s, java.util.Vector vector) {
    try {
      String server = null;
      //
      // check if the string passed in is a URL or a server. if it is a URL,
      // parse out
      ...
      while (e.hasMoreElements()) {
        ...
        if (o.getClass().getName().equals(RemoteClassName)) {
          String[] list = ((com.borland.jdbc.Driver) o)
              .getDataSourcesList(server);
   ...
   * DataGateway. The database doesn't have to be open at the time, but its
   * driver should have been registered already.
   * 
   * @param database
   *          is the Database object to be tested
   * 
