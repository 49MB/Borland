...
/**
 * This class is intended to provide basic metadata information and some DDL
 * functionality.
 */
...
public class MetaData extends RuntimeMetaData {
  ...
  /**
   * Gets an Enumeration of String objects representing the existing ODBC Data
   * Sources. Later the existing Velcro aliases will be added.
   */
  public static Enumeration<Object> getURLs() {
  ...
  public static Enumeration<Object> getBorlandLocalURLs() {
    Vector<Object> result = new Vector<Object>();
    BorlandMetaData.getBorlandLocalURLs(result);
    return (result.elements());
  }
  
  public static Enumeration<Object> getBorlandRemoteURLs(String serverName) {
    Vector<Object> result = new Vector<Object>();
  ...
  /**
   * Gets an Enumeration of String objects representing the existing tables in
   * the current database.
   */
  public Enumeration<String> getTables() throws MetaDataException {
    String userName = getUserName();
    
    Vector<String> result = new Vector<String>();
    try {
      ResultSet tables = getJdbcMetaData().getTables(null, null, "%",
          new String[] { "TABLE" }); // NORES
      while (tables.next()) {
        ...
        if (userName == null || owner == null
            || userName.equalsIgnoreCase(owner))
  ...
  /**
   * Gets an Enumeration of String objects representing the existing procedure
   * names in the current database.
   */
  public Enumeration<String> getProcedures() throws MetaDataException {
    String userName = getUserName();
    ...
    try {
      ...
      while (procedures.next()) {
        String owner = trimRight(procedures.getString(2)); // "PROCEDURE_SCHEM"
                                                           // //NORES
        String name = trimRight(procedures.getString(3)); // "PROCEDURE_NAME"
                                                          // //NORES
        if (name != null && name.indexOf((int) ';') > 0)
          name = name.substring(0, name.indexOf((int) ';')); // Hack for the
                                                             // thin Sybase
                                                             // driver (JOAL)
  ...
  /**
   * Get the columns of a procedure from the DatabaseMetaData information. Store
   * the columns in a hashtable for quick re-retrieval.
   * 
   * @param procedureName
   *          the name of the procedure of interest.
  ...
  public synchronized ParameterRow getProcedureColumns(String procedureName)
    ...
    try {
      ResultSet procedures = getJdbcMetaData().getProcedureColumns(null,
          names[0], names[1], "%"); // NORES
      while (procedures.next()) {
        String name = trimRight(procedures.getString(4)); // "COLUMN_NAME"
                                                          // //NORES
        ...
        Column column = createColumn(name, name, sqlType, precision, 0, scale,
            false);
  ...
  /**
   * Gets an Enumeration of String objects representing the column names of a
   * table.
   */
  public Enumeration<String> getColumns(String tableName)
      throws MetaDataException {
    String[] names = splitName(tableName);
    
    Vector<String> result = new Vector<String>();
    try {
      ResultSet fields = getJdbcMetaData().getColumns(null, names[0], names[1],
          "%"); // NORES
      while (fields.next()) {
        result.addElement(trimRight(fields.getString(4))); // "COLUMN_NAME"
                                                           // //NORES
      }
      fields.close();
    } catch (SQLException ex) {
  ...
  /**
   * @deprecated Use createTable(TableDescriptor) instead. Create a Table with
   *             the columns described by the given dataSet.
   */
   ...
   *             The primaryKey may be either of the 5 formats: 1) String[] With
   *             column names. 2) Number[] With indices into the columns of the
   *             passed dataSet. 3) int[] With indices into the columns of the
   *             passed dataSet. 4) Object[] A hybrid of column names and column
   *             indices. 5) Enumeration A hybrid of column names and column
   *             indices.
   */
  public void createTable(String tableName, DataSet dataSet, Object primaryKey)
    ...
    if (primaryKey != null) {
      ...
      table.indexes[0] = new SortDescriptor(tableName + "_PK",
          makeFieldNameArray(primaryKey, dataSet), null, null, 0);
    }
    createTable(table);
  }
   ...
   */
  public void dropTable(TableDescriptor table) throws MetaDataException {
    try {
   ...
   * Drop the given Table from the current database and ignore failures.
   */
  public void dropTableIfExist(String tableName) throws MetaDataException {
    try {
      dropTable(tableName);
    } catch (DataSetException ex) {
    }
   ...
   */
  public void dropTable(String tableName) throws MetaDataException {
   ...
   */
  public boolean tableExists(String tableName) throws MetaDataException {
    try {
   ...
   * 
   *             Create an index with the name 'indexName' on the table
   *             'tableName'. unique if the index should be unique. The
   *             indexFields may be either of the 5 formats: 1) String[] With
   *             column names. 2) Number[] With indices into the columns of the
   *             passed dataSet. 3) int[] With indices into the columns of the
   *             passed dataSet. 4) Object[] A hybrid of column names and column
   *             indices. 5) Enumeration A hybrid of column names and column
   *             indices.
   */
  public void createTableIndex(String indexName, String tableName,
      boolean unique, Object indexFields, DataSet dataSet) {
    SortDescriptor index = new SortDescriptor(indexName, makeFieldNameArray(
        indexFields, dataSet), null, null, 0);
    TableDescriptor table = new TableDescriptor();
    table.table = tableName;
   ...
   * 
   * @param table
   *          description of table to create index for.
   * @param index
   *          description of of index to create.
   */
  
  public void createTableIndex(TableDescriptor table, SortDescriptor index) {
    // try {
    String sql = makeSqlForCreateIndex(table, index);
    Diagnostic.trace(Trace.MetaData, "SQL for Create Index:  " + sql); // NORES
    database.executeStatement(sql);
    Diagnostic.trace(Trace.MetaData, "Index " + index.getIndexName()
        + " created!"); // NORES
    // }
    // catch(SQLException ex) {
    // MetaDataException.rethrowSQLException(ex);
  ...
  public static DataSetAnalyzer getDataSetAnalyzer(DataSet dataSet) {
    ...
    if (storageDataSet instanceof QueryDataSet || provider != null
        && provider instanceof QueryProvider)
  ...
  Column createColumn(String schemaName, String tableName, String columnName) {
    Column col = null;
    try {
      ResultSet fields = getJdbcMetaData().getColumns(null, schemaName,
      ...
      if (fields.next()) {
        ...
        boolean search = true;
        col = createColumn(columnName, label, sqlType, precision, 0, scale,
            search);
      }
      fields.close();
    } catch (Exception ex) {
  ...
  private Enumeration<Object> getEnumeration(Object list)
      throws MetaDataException {
    if (list instanceof Enumeration)
      return (Enumeration<Object>) list;
  ...
  private String[] makeFieldNameArray(Object fieldList, DataSet dataSet)
    ...
    Enumeration<Object> fieldIterator = getEnumeration(fieldList);
    while (fieldIterator.hasMoreElements()) {
      Object obj = fieldIterator.nextElement();
      if (obj instanceof String)
        list.addElement(obj);
      else if (obj instanceof Number)
        list.addElement(dataSet.getColumn(((Number) obj).intValue())
            .getColumnName());
    }
  ...
  private String createTableSql(TableDescriptor table) throws MetaDataException {
    if (implementor == null)
      getImplementor();
    ...
    try {
      ...
      for (int i = 0; i < columns; i++) {
        {// if (dataSet.getColumn(i).getCalcType() == CalcType.NO_CALC) {
          ...
          sql += column.getColumnName() + " "
              + implementor.columnToSQLDataType(column); // NORES
      ...
      if (table.primaryKey != null) {
        sql += ", PRIMARY KEY("
            + makeFieldNameListString(table.primaryKey.getKeys()) + ")"; // NORES
      }
      sql += ")"; // NORES
    } catch (DataSetException ex) {
      MetaDataException.rethrowDataSetException(ex);
  ...
  private String makeSqlForCreateIndex(TableDescriptor table,
      SortDescriptor index) throws MetaDataException {
    String sql = index.isUnique() ? "CREATE UNIQUE INDEX " : "CREATE INDEX "; // NORES
    sql += index.getIndexName() + " ON " + table.table + "("
        + makeFieldNameListString(index.getKeys()) + ")"; // NORES
    return sql;
  }
  ...
  public boolean isSqlTypeSupported(short sqlType) {
    switch (sqlType) {
    case java.sql.Types.CHAR: // case statements from:
  ...
  public String getProcedureQueryString(String procedureName,
      boolean forResultSet, boolean[] warnings) {
    return getImplementor().getProcedureQueryString(procedureName,
        forResultSet, warnings);
  }
...
class ObjectArrayEnumerator implements Enumeration<Object> {
...
class IntArrayEnumerator implements Enumeration<Object> {
