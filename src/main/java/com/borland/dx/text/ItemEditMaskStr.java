...
//    Diagnostic.addTraceCategory(Trace.EditMaskStr);
    ...
    if (formatter == null) {
      Diagnostic.trace(Trace.EditMaskStr, "Null formatter -- making our own with editMask: " + editMask);
      ...
      if (this.formatter == null)
        Diagnostic.trace(Trace.EditMaskStr, "failed");
    }
    ...
    this.editMask = (editMask == null || editMask.length() == 0)
                    ...
                    : editMask;
    Diagnostic.trace(Trace.EditMaskStr, "ItemEditMaskStr: using pattern: " + this.editMask);
    ...
    if (this.editMask == null || this.editMask.length() == 0) {
      Diagnostic.trace(Trace.EditMaskStr, "null editmask making RegionAny");
      noPattern = true;
...
//!System.err.println("blankchar is <" + blankChar + ">");
      ...
      default:
          Diagnostic.trace(Trace.EditMaskStr, "Didn't decode " + formatterType);
  ...
  public ItemEditMaskState prepare(Variant value) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "ItemEditMaskStr.prepare(" + editMask + ", " + startCursor + ")");
//!System.err.println("ItemEditMaskStr.prepare(" + value + ")");
  ...
  public boolean insert(ItemEditMaskState state, char c) {
    ...
    int ic = (int) c;
    Diagnostic.trace(Trace.EditMaskStr, "Insert(" + c + ") whose value is " + ic);
    //
    ...
    if (noPattern == false && allowLeftShift && state.cursorPos >= lastEditPos) {
      Diagnostic.trace(Trace.EditMaskStr, "We are now shifting left...");
      shiftLeft(state);
    ...
    if (isValid(state.cursorPos, c)) {
      Diagnostic.trace(Trace.EditMaskStr, "Storing the character...");
      setCharAt(state.displayString, state.cursorPos, c);

      Diagnostic.trace(Trace.EditMaskStr, "Advancing cursor...");
    ...
    //
      ...
      Diagnostic.trace(Trace.EditMaskStr, "Swallowing an auto-skipped literal");
      autoSkipLiteral = 0;
      return true;
    }
    else if (autoSkipLiteral != c) {
      ...
      Diagnostic.trace(Trace.EditMaskStr, "Looking to see if can skip to this literal");
      //!TODO for (i = state.cursorPos; i <= lastEditPos /* TODO!regionMap.length*/; ++i) {
      for (i = state.cursorPos; i <= lastEditPos ; ++i) {
        if (isLiteral(i)) {
          Diagnostic.trace(Trace.EditMaskStr, "  considering literal at position " + i);
//!          eml = (ItemEditMaskRegionLiteral) editRegions.elementAt(regionMap[i]);
          eml = (ItemEditMaskRegionLiteral) getRegionFromPosition(i);
          if (eml.capacity != 1) {
            Diagnostic.trace(Trace.EditMaskStr, "  skipping, literal too long");
            continue;
          }
          //if (c == state.displayString.charAt(i))
          if (c == eml.literal.charAt(0)) {
            state.cursorPos = i;
            Diagnostic.trace(Trace.EditMaskStr, "  yes -- moving to position " + i);
            internalMove(state, KeyEvent.VK_RIGHT); //! /*Event.RIGHT*/);
            return true;
          }
          else {
            ic = (int) eml.literal.charAt(0);
            Diagnostic.trace(Trace.EditMaskStr, " hmmm... skipped literal was \"" + eml.literal.toString() + " which is value " + ic);
          }
        }
      }
    }
    ...
    //
    Diagnostic.trace(Trace.EditMaskStr, "Insert failed: char = \'" + c + "\' and autoskiplit = \'" + autoSkipLiteral + "\'");
    return false;
  }
  ...
  public boolean delete(ItemEditMaskState state, int startPos, int count) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "Delete: from " + startPos + " for " + count + " chars");
    if (startPos < 0)
      startPos = 0;
    for (int pos = startPos; pos <= lastEditPos && count > 0; pos++, --count) {
      ...
      Diagnostic.trace(Trace.EditMaskStr, "  deleting char as pos " + pos);
  ...
  public boolean isComplete(ItemEditMaskState state) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "ItemEditMaskStr.isComplete()");
  ...
  public void getFinalValue(ItemEditMaskState state, Variant value) throws InvalidFormatException {
    Diagnostic.trace(Trace.EditMaskStr, "ItemEditMaskStr.getFinalValue() short form");
  ...
  public void getFinalValue(ItemEditMaskState state, Variant value, int variantType) throws InvalidFormatException {
    Diagnostic.trace(Trace.EditMaskStr, "ItemEditMaskStr.getFinalValue() long form");
    getVariantFromString(state, value, variantType);
  }
  ...
  public boolean isOptional(int charPosition) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "  EditMaskStr reporting back " + optional);
    return optional;
  }

  public char setCharAt(StringBuffer str, int charPosition, char c) {
    if (charPosition < 0 || charPosition > lastEditPos) {
      Diagnostic.trace(Trace.EditMaskStr, "setCharAt: pos = " + charPosition + " and end = " + lastEditPos);
      return c;
    }
    ItemEditMaskRegion emr = getRegionFromPosition(charPosition);
    Diagnostic.trace(Trace.EditMaskStr, "setCharAt using region " + emr.getClass().getName());
  ...
  */
  ...
  private boolean handleSign(ItemEditMaskState state, int charPos, char c, boolean inserting) {
    Diagnostic.trace(Trace.EditMaskStr, "handleSign at pos " + charPos);
    ItemEditMaskRegion emr = getRegionFromPosition(charPos);
    Diagnostic.trace(Trace.EditMaskStr, "  region is of class " + emr.getClass().getName());
    if (!(emr instanceof ItemEditMaskRegionSign))
      return false;

    Diagnostic.trace(Trace.EditMaskStr, "Inserting \'" + c + "\' at position " + state.cursorPos + " is a sign region");

    // If not valid, advance cursor off the sign character so user can just start typing digits
    if (!isValid(charPos, c)) {
      Diagnostic.trace(Trace.EditMaskStr, " not valid");
      if (inserting)
        internalMove(state, KeyEvent.VK_RIGHT);
      return false;
    }
    Diagnostic.trace(Trace.EditMaskStr, " valid, putting char <" + c + ">");
    ...
    for (i = 0; i <len; ++i) {
      ...
      Diagnostic.trace(Trace.EditMaskStr, "found other sign at pos " + i + ", making char to be <" + c + ">");
...
//!System.err.println("    variant.setDate = " + v);
    ...
    Diagnostic.trace(Trace.EditMaskStr, "DateTimeFormat(" + dateVal.toString() + ", " + pattern + ") yields: " + s);
//!System.err.println("  DateTimeFormat(" + dateVal.toString() + ", " + pattern + ") yields: " + s);
  ...
  private final boolean DateTimeParse(String dateString,
    ...
    Diagnostic.trace(Trace.EditMaskStr, "DateTimeParse: parsing \"" + dateString + " \" with \"" + pattern + "\"");
...
//e.printStackTrace(System.err);
    ...
    Diagnostic.trace(Trace.EditMaskStr, "DateTimeParse: returning " + value);
    return true;
  }
  ...
  private final Number NumericParse(String numString, String pattern, ItemEditMaskState state) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "NumericParse: parsing " + numString + " with " + pattern);
...
//e.printStackTrace(System.err);
    ...
    if (v != null) {
      Diagnostic.trace(Trace.EditMaskStr, "  got a variant back of type " + v.getType());
...
//!System.err.println("BooleanParse: formatter returned " + v);
  ...
  private boolean isRegionComplete(ItemEditMaskState state, int nRegion) {
    Diagnostic.trace(Trace.EditMaskStr, "isRegionComplete(" + nRegion + ")");
//!System.err.println("isRegionComplete(" + nRegion + ")");
    ...
    if (emr instanceof ItemEditMaskRegionLiteral) {
      Diagnostic.trace(Trace.EditMaskStr, " literal -- return true");
      return true;
    }
    if (emr.optional) {    // quick out for fully optional regions
      Diagnostic.trace(Trace.EditMaskStr, " optional -- return true");
      return true;
    }
...
//!System.err.println("offset= " + emr.offset + ", capacity=" + emr.capacity + ", minReq=" + emr.minRequired);
    ...
    for (i = emr.offset; i < emr.offset + emr.capacity; ++i) {
      ...
      if (emr.minRequired == 0) {  // minRequired==0 means the region is fixed size and knows for each char position
        Diagnostic.trace(Trace.EditMaskStr, "  minRequired 0 -- setting cursorPos to " + i);
        state.cursorPos = i;  // meaning ANY missing characters for this kind of region means incomplete
//!System.err.println("bailing out");
        return false;
      }
    }
    Diagnostic.trace(Trace.EditMaskStr, "Number of required characters left blank = " + nBlanks);
...
//$$$    state.cursorPos = (emr.rightToLeft) ? emr.offset + emr.capacity - emr.minRequired : emr.offset + emr.minRequired;
    ...
    Diagnostic.trace(Trace.EditMaskStr, "  offset = " + emr.offset + ", capacity = " + emr.capacity + ", minReq = " + emr.minRequired);
    Diagnostic.trace(Trace.EditMaskStr, " setting cursorPos to " + state.cursorPos);
    return false;
  }
  ...
  private final void localizeSymbols() {
    try {
      ...
      Diagnostic.trace(Trace.EditMaskStr, "Decimal pt = \'" + decimalSign + "\', and thousands sep = \'" + thousandsSign + "\'");
  ...
  private final int moveCursorOffLiteral(int cursorPos) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "MoveCursorOffLiteral, pos = " + cursorPos + ", and len = " + iLen);
    ...
    for (forwardPos = cursorPos+1, backwardPos = cursorPos - 1; ; ++forwardPos, --backwardPos) {
      ...
      if (forwardPos < iLen) {
        if (!isLiteral(forwardPos))
          return forwardPos;
          else Diagnostic.trace(Trace.EditMaskStr, "pos " + forwardPos + " is a literal");
      }
      if (backwardPos > 0) {
        if (!isLiteral(backwardPos))
          return backwardPos;
          else Diagnostic.trace(Trace.EditMaskStr, "pos " + backwardPos + " is a literal");
      }
    }
  ...
  private final int charPositionsToHere(int iEditRegion) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "Cursor position at " + nChars);
    return nChars;
  }
  ...
  private final void buildDateTimeRegions() {
    ...
    for (c = editChars.firstChar(); c != FastStringBuffer.NOTACHAR; c = editChars.nextChar()) {
      Diagnostic.trace(Trace.EditMaskStr, "buildDateTimeRegions processing \'" + c + "\' at pos " + editChars.getOffset());
      //
      ...
      switch (c)
      {
        ...
        case '^':
          ...
          Diagnostic.trace(Trace.EditMaskStr, "Starting cursor is " + startCursor);
          continue;
          //
        ...
        case 'z':
          ...
          while ((nextC = editChars.nextChar()) == c)
            ++charCount;
          Diagnostic.trace(Trace.EditMaskStr, " counted " + charCount + " of the letter " + c);
          ...
          switch (c)
          {
            ...
            case 'z':
              ...
              break;
            default:
                Diagnostic.trace(Trace.EditMaskStr, "Missing case in 2nd switch of BuildDateTimeRegions");
          }
          continue;

        case '\'':
          workBuffer.empty();
          Diagnostic.trace(Trace.EditMaskStr, "See single quote, pos is " + editChars.getOffset());
          FastStringBuffer fsb = editChars.parseLiteral(c, true);   // allow double quote
          Diagnostic.trace(Trace.EditMaskStr, "End single quote, pos is " + editChars.getOffset());
          workBuffer.append(fsb.value(), 0, fsb.length());
          Diagnostic.trace(Trace.EditMaskStr, "adding literal region: " + workBuffer);
          editRegions.addElement(new ItemEditMaskRegionLiteral(this, workBuffer));
          workBuffer.empty();
          litCount = 0;
          continue;
  ...
  private final void buildNumericRegions() {
    ...
    for (c = editChars.firstChar(); c != FastStringBuffer.NOTACHAR; c = editChars.nextChar()) {
      ...
      // We are here only for parsing the 1st subfield -- the editMask itself.  'c' contains the next char.
      //
      Diagnostic.trace(Trace.EditMaskStr, "buildNumericRegion: char = \'" + c + "\'");
      switch (c)
      {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "End of BuildNumericRegion: negative suffix is " + s);
    if (df != null && s.length() > 0) {
        Diagnostic.trace(Trace.EditMaskStr, "Found suffix " + s.length() + " chars long, previous literal was " + litCount + " chars long");
      if (s.length() > litCount) {
        Diagnostic.trace(Trace.EditMaskStr, "So we are adding a suffix negative sign region");
        ...
        else Diagnostic.trace(Trace.EditMaskStr, "So we are NOT adding a suffix sign");
    }
  }
  ...
  private final boolean internalMove(ItemEditMaskState state, int keyCode) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "internalMove: now at pos " + state.cursorPos);
...
//!System.err.println("default -- out");
    ...
    Diagnostic.trace(Trace.EditMaskStr, " initial move took us to pos " + pos);
    //
...
//        ItemEditMaskRegionLiteral emr = (ItemEditMaskRegionLiteral) editRegions.elementAt(regionMap[pos]);
        ...
        if (emr != null && emr.capacity == 1) {
          ...
          Diagnostic.trace(Trace.EditMaskStr, "Detected autoskip literal \'" + autoSkipLiteral + "\'");
        }
        else autoSkipLiteral = 0;
        continue;
      }
      Diagnostic.trace(Trace.EditMaskStr, " final move took us to pos " + pos);
      state.cursorPos = pos;
      return true;
    }
  ...
  private final void buildTextRegions() {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "BuildTextRegions using the pattern \"" + editChars.toString() + "\"");

    for (c = editChars.firstChar(); c != FastStringBuffer.NOTACHAR; c = editChars.nextChar()) {
      Diagnostic.trace(Trace.EditMaskStr, "BuildTextRegion: char is '" + c + "'");
      //
      // We are here only for parsing the 1st subfield -- the editMask itself.  'c' contains the next char.
      //
      Diagnostic.trace(Trace.EditMaskStr, "BuildTextRegion fetched '" + c + "'");
        ...
        case '*':
          ...
          switch (c)
          {
            case '^':
              ...
              Diagnostic.trace(Trace.EditMaskStr, "Text Starting cursor is " + startCursor);
              Diagnostic.trace(Trace.EditMaskStr, "editRegionSize = " + editRegions.size() + ", charCount = " + charCount);
              break;
            case '<':
              caseConvert = -1;
              break;
        ...
        default:
          ...
          switch (c)
          {
            ...
            default:
              ...
              workBuffer.append(c);
                Diagnostic.trace(Trace.EditMaskStr, "  it goes into pos " + litCount + "in the literal buffer");
              litCount++;
              continue;
          }
  ...
  private final char[] buildEmptyEditCharArray() {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "buildEmptyEditCharArray()");

    for (i = 0; i < buflen; ++i) {
      Diagnostic.trace(Trace.EditMaskStr, "i = " + i + ", Fillpos = " + fillPos + ", and buffer length is " + buffer.length);
      //emr = (ItemEditMaskRegion) editRegions.elementAt(regionMap[i]);
      emr = getRegionFromPosition(i);
      Diagnostic.trace(Trace.EditMaskStr, "  next region is " + emr.getClass().getName());
      if (emr.isLiteral(fillPos)) {
        eml = (ItemEditMaskRegionLiteral) emr;
        j = eml.capacity;
          Diagnostic.trace(Trace.EditMaskStr, "Filling " + j + " chars from literal " + eml.literal.toString());
    ...
    Diagnostic.trace(Trace.EditMaskStr, "buildEmptyEditCharArray() returning: " + new String(buffer));
    return buffer;
...
//!System.err.println("buildDataTimeEditString(" + dateValue + ")");
    ...
    Diagnostic.trace(Trace.EditMaskStr, "buildDateTimeEditString(" + dateValue + ")");

    char[] buffer = buildEmptyEditCharArray();  // fill in '_' chars and literals

    Diagnostic.trace(Trace.EditMaskStr, "empty edit string = " + new String(buffer));
...
//      emr = (ItemEditMaskRegion) editRegions.elementAt(regionMap[i]);
      ...
      if (!(emr instanceof ItemEditMaskRegionLiteral)) {
        ...
        Diagnostic.trace(Trace.EditMaskStr, "  injecting " + str);
...
//!System.err.println(" building editMask value using pattern " + trueEditMask + " for value " + absValue + ", negative = " + negative);
    ...
    Diagnostic.trace(Trace.EditMaskStr, " absolute formatted as: <" + s + ">");
...
//!    */
  ...
  private final FastStringBuffer getFinalEditString(ItemEditMaskState state) {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "getFinalEditString = \"" + s + "\" and length is " + iLen);
...
//!System.err.println("isEmptyString: length is " + state.displayString.toString().length());
  ...
  private final void getVariantFromString(ItemEditMaskState state,
    ...
    Diagnostic.trace(Trace.EditMaskStr, "GetVariantFromString: string is \"" + state.displayString + "\" and type wanted is " + variantType);
    if (variantType <= Variant.NULL_TYPES)
      variantType = this.variantType;

    Diagnostic.trace(Trace.EditMaskStr, "getVariantFromString into type " + Variant.typeName(variantType));
    ...
    Diagnostic.trace(Trace.EditMaskStr, "done with isComplete() call");
    //
    ...
    Diagnostic.trace(Trace.EditMaskStr, "empty buffer, assigning null");
    ...
    switch (formatterType)
    {
      ...
      case VariantFormatter.DATETIME:
        ...
        if (!success) {
          Diagnostic.trace(Trace.EditMaskStr, "getVariantFromString throwing exception");
          throw VariantFormatStr.makeInvalidFormatException(editMask, state.cursorPos);
        }
...
//!        String resultStr = TextParse(s, blankChar, replaceBlanksWith /*VariantFormatter.NOTACHAR*/);
      ...
      default:
        Diagnostic.trace(Trace.EditMaskStr, "getVariantFromString: unknown type " + variantType + ", assigning null");
        value.setNull(Variant.ASSIGNED_NULL);  // should never get here, but return blank variant if so
    }
    Diagnostic.trace(Trace.EditMaskStr, "returning from getVariantFromString");
  }
  ...
  private void buildRegionMap() {
    ...
    Diagnostic.trace(Trace.EditMaskStr, "BuildRegionMap: there are " + editRegions.size() + " regions");
    ...
    for (i = 0; i < editRegions.size(); ++i) {
      ...
      Diagnostic.trace(Trace.EditMaskStr, "  region " + i + "has " + emr.capacity + " slots");
      capacity += emr.capacity;
    }

    Diagnostic.trace(Trace.EditMaskStr, "will require total of " + capacity + " slots");
    ...
    for (i = 0; i < editRegions.size(); ++i) {
      emr = (ItemEditMaskRegion) editRegions.elementAt(i);
      Diagnostic.trace(Trace.EditMaskStr, "Region " + i + " has " + emr.capacity + " characters");
      ...
      if (anyChanges) {
        ...
        for (i = 0; i < iLen; ++i) {
          ...
          if ((i >= posSrc && i < decimalAt) || (i > decimalAt && i <= posSrc)) {
            Diagnostic.trace(Trace.EditMaskStr, "pos " + i + " is between " + posSrc + " and " + decimalAt);
            emn = getRegionFromPosition(i);
            if (emn instanceof ItemEditMaskRegionNumeric) {
              Diagnostic.trace(Trace.EditMaskStr, "blanking it");
