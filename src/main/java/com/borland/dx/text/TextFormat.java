...
public TextFormat (String pattern) {
  ...
  private void decomposePattern() {
    ...
    Diagnostic.trace(TRACE, "Decomposing pattern: " + pattern);
    ...
    for (c = fsb.firstChar(); c != FastStringBuffer.NOTACHAR; c = fsb.nextChar()) {
      switch (c)
      {
        ...
        case ';':
          ...
          Diagnostic.trace(TRACE, "   next char is \'" + c + "\'");
          if (c == ';')
            c = 0;
    ...
    if (injectLiterals)
      ...
      Diagnostic.trace(TRACE, "We are injecting " + litCount + " literals");
    if (rightToLeft)
      Diagnostic.trace(TRACE, "We are scanning right to left");
    Diagnostic.trace(TRACE, "The fillChar is \'" + fillChar + "\'");
    Diagnostic.trace(TRACE, "The replaceChar is \'" + replaceChar + "\'");
    ...
    Diagnostic.trace(TRACE, "Pattern has " + litCount + " literals and " + charCount + " char slots");
    ...
    for (c = fsb.firstChar(); c != FastStringBuffer.NOTACHAR; c = fsb.nextChar()) {
      Diagnostic.trace(TRACE, "considering char " + c);
      switch(c)
      {
        ...
        default:        // everything else is a literal
          Diagnostic.trace(TRACE, "Litmask[" + fillPos + "] gets the char \'" + c + "\'");
          infoMask[fillPos] = LITERAL_CHAR;
          litMask[fillPos++] = c;
          continue;
      }
    ...
    Diagnostic.trace(TRACE, "LitMask is " + litMask.length + " and we filled " + fillPos);
 ...
 */
  public StringBuffer format(String toBeFormatted, StringBuffer result, FieldPosition pos) {
    Diagnostic.trace(TRACE, "format(" + toBeFormatted + ", " + result + ", " + pos + ")");
    ...
    Diagnostic.trace(TRACE, "Formatting \"" + toBeFormatted + "\" with \"" + pattern + "\""); //$$$
...
//!    */
    ...
    Diagnostic.trace(TRACE, "iLen = " + iLen + ", maxChars = " + maxChars + ", charCount = "
...
//!     */
    ...
    Diagnostic.trace(TRACE, "Input buffer is " + buffer.length + ", FillBuffer is " + fillBuffer.length);
    if (litMask != null)
      Diagnostic.trace(TRACE, "litMask is " + litMask.length);

    for (fillPos = i; i >= 0 && i < maxChars; i += incr) {
      Diagnostic.trace(TRACE, "FillPos=" + fillPos + ", i=" + i + ", nRemaining=" +nRemaining);  //$$$
...
//!*/
      ...
      if (nRemaining <= 0) {
        ...
        if (isLiteral(i)) {
          Diagnostic.trace(TRACE, "nRemaining <= 0, filling with litMask " + i  + " at pos " + fillPos + ": " + litMask[i]); //$$$
          Diagnostic.trace(TRACE, " pads with literal \'" + litMask[i] + "\'");
        ...
        if (replaceChar != 0) {
          Diagnostic.trace(TRACE, "nRemaining <= 0, filling with replaceChar at pos " + fillPos); //$$$
...
//!//          }
//!//        }
        else Diagnostic.trace(TRACE, " being skipped");  //$$$
        continue;
      }
      //
      ...
      if (isLiteral(i)) {
        Diagnostic.trace(TRACE, "literal <" + litMask[i] + "> expected at this position");
        if (injectLiterals) {
          Diagnostic.trace(TRACE, " [" + fillPos + "] gets the literal \'" + litMask[i] + "\'");
          fillBuffer[fillPos] = litMask[i];
          fillPos += incr;
        }
        //
        ...
        // format something which does not fit the mask, so the best we can do is try to help.
          ...
          Diagnostic.trace(TRACE, "Considering pos " + srcPos + ": " + buffer[srcPos]);  //$$$
          if (buffer[srcPos] != litMask[i] && ((infoMask[i] & PASSWORD_CHAR) == 0)) {
            Diagnostic.trace(TRACE, "Yikes: expected to see literal /'" + litMask[i] + "\' here");
            Diagnostic.trace(TRACE, "  but saw the char '\'" + buffer[srcPos] + " '\'");
            Diagnostic.trace(TRACE, "  injecting literal anyway to try to recover");
            Diagnostic.trace(TRACE, "  pos " + srcPos + " != literal: filling with replaceChar");
            if (replaceChar != 0) {
            ...
            if (isPassword(i)) {
              Diagnostic.trace(TRACE, " [" + fillPos + "] gets password fill");
              fillBuffer[fillPos] = '*';
            }
            else {
              Diagnostic.trace(TRACE, " [" + fillPos + "] copies \'" + buffer[srcPos] + "\' from input text");
      ...
      //
        ...
        Diagnostic.trace(TRACE, " (lookahead lit = \'" + nextLit + "\' <" + (int) nextLit + ">)"); //$$$
...
//!//!RC          --nRemaining;      // Bug fix 7/16/97 Don't decrement remaining chars if filling with empties
        ...
        if (litMask != null && ((infoMask[i] & PASSWORD_CHAR) != 0)) {
          ...
          Diagnostic.trace(TRACE, " [" + fillPos + "] copies '" + buffer[srcPos] + "' from text");
          fillBuffer[fillPos] = buffer[srcPos];
          srcPos += incr;
        }
        fillPos += incr;
...
//!*/
    ...
    Diagnostic.trace(TRACE, "returning " + result);
    return result;
  }
 ...
 */
  public StringBuffer parse(String text, ParsePosition pos) {
    ...
    int startPos;
    char[] fillBuffer = new char[maxChars];
    Diagnostic.trace(TRACE, "The text " + text + " is " + iLen + " and we will fill into " + fillBuffer.length);

    int nFilled = 0;
    int fillPos = 0;
    char c;
    if (!injectLiterals || litCount == 0) {
      Diagnostic.trace(TRACE, "No literal stripping -- removing up to " + iLen + " chars from " + text);
      for(i = 0; i < maxChars; ++i) {
        Diagnostic.trace(TRACE, "Buffer[" + i + "]");
        ...
        if (litMask != null && litMask[i] != NOT_A_CHAR && ((infoMask[i] & PASSWORD_CHAR) == 0)) {                      // literals copy thru directly
          Diagnostic.trace(TRACE, " filling literal " + litMask[i]);
        ...
        Diagnostic.trace(TRACE, "  appending " + c);
        fillBuffer[fillPos++] = c;
      }
      Diagnostic.trace(TRACE, "And the final buffer has " + fillPos + " chars");
    ...
    for (; i >= 0 && i < iLen; i += incr, litPos += incr) {
      ...
      if (litMask[litPos] == NOT_A_CHAR || ((infoMask[litPos] & PASSWORD_CHAR) != 0)) {
        c = buffer[i];
        Diagnostic.trace(TRACE, "   put '" + c + "' into fillBuffer[" + fillPos + "] and iLen = " + iLen);
        --nRemaining;
        if (c == fillChar) {
          Diagnostic.trace(TRACE, "     it is a fillChar and gets translated to" + replaceChar);
          c = replaceChar;
          if (c == 0) {
