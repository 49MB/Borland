 ...
 * <li>Column level read/write operations for and class that extends
 * ReadWriteRow.</li>
 ...
 * <p>
 * If RowIterator is bound to a DataSet, the post() method must be used to cause
 * the changes to appear in the bound DataSet. This allows multiple RowIterators
 * to be simultaneously editing differnt rows in the same DataSet. When bound to
 * a ReadWriteRow, setting columns takes immediate affect. You can force a
 * DataSet to be bound as a ReadWriteRow by calling
 * RowIterator.bind((ReadWriteRow)DataSet);
 * <p>
 * If you have bound an RowIterator to a DataSet, you must call
 * RowIterator.unbind() to free up allocated memory resources used by the
 * RowIterator. Note that this does not have to be done if the DataSet was bound
 * as a ReadWriteRow (ie RowIterator.bind((ReadWriteRow)DataSet);).
 * <p>
 ...
 * Once established, tools like JBuilder can provide nice accessor choices
 * inside the code editor via code insights. Take a customer table for example.
 * In this case an application may set up an entity Object that extends
 * StorageDataSet. The entity Object contains the business logic for customers
 * expressed as property and event settings with associated business logic. The
 * entity object has a private jbInit() method so it is designable using tools
 * like JBuilder. The jbInit() method contains the persistent columns, property
 * and event settings for the entity object.
 */
...
public class RowIterator extends ReadWriteRow {
  ...
  /*-throws DataSetException-*/
  {
    if (dataSet != null) {
      ...
      //
      if (readRow != dataSet
          || !(editView != null && dataSet.dataSetStore == editView.dataSetStore))
        closeEditView();
    }
    this.readRow = row;
  ...
  /*-throws DataSetException-*/
  {
    if (iterator.dataSet != null) {
      ...
      synchronized (dataSet.getSyncObj()) {
  ...
  /**
   * Bind to a DataSet. unbind() must be called when you are done with this
   * iterator. set/get operations allowed on Columns. Row navigation allowed.
  ...
  /**
   * Binds to a ReadRow. Once the RowIterator is bound, get operations are
   * allowed on Columns.
   * 
   * @param readRow
   */
  ...
  /*-throws DataSetException-*/
  {
    if (dataSet == null)
      ...
      if (editing && !columnList.cols[ordinal].isLookupOrAggregate()) {
        Diagnostic.check(rowValues == editView.rowValues);
        return rowValues[ordinal];
      } else {
        if (currentRow == 0 && dataSet.getRowCount() < 1)
        ...
        return dataSet.dataSetStore.getVariantStorage(this, internalRow,
            ordinal, value);
      }
    }
  }
  ...
  /*-throws DataSetException-*/
  {
    if (dataSet == null)
      return true;
    return currentRow == (dataSet.getRowCount() - 1)
        && !dataSet.dataSetStore.hasMoreData();
  }
  ...
  /**
   * Move to first row visible by this dataSet. This may cause the inBounds() to
   * return true if more than one row exists.
   */
  ...
  /**
   * Move to last row visible by this dataSet. This may cause the inBounds() to
   * return true if more than one row exists.
   */
  public final void last() /*-throws DataSetException-*/{
    ...
    if (dataSet != null) {
      ...
      synchronized (dataSet.getSyncObj()) {
        ...
        if (editing)
          _post();
        _goToRow(dataSet.getRowCount() - 1);
      }
    }
  }
  ...
  /**
   * Move to the next row visible by this iterator. This will cause the
   * inBounds() to return false if next is called when the iterator is
  ...
  /**
   * Move to the prior row visible by this iterator. This will cause the
   * inBounds() to return false if prior is called when the iterator is
   * positioned at the first visible row.
   */
  ...
  /**
   * Will throw a DataSetException if not bound to a DataSet. Associates the
   * Iterator with an unposted row with default values. Columns that have no
   * default values, are initialized with Variant.UNASSIGNED_NULL.
   */
  ...
  /**
   * Will throw a DataSetException if not bound to a DataSet. Deletes row that
   * RowIterator is positioned at.
   */
  ...
  /*-throws DataSetException-*/
  {
    if (editView == null) {
      ...
      synchronized (dataSet.getSyncObj()) {
        ...
        if (dataSet.getRowFilterListeners() != null) {
          editView.addRowFilterListener(dataSet.getRowFilterListeners());
        }
        editView.open();
      }
    }
  ...
  /**
   * Will throw a DataSetException if not bound to a DataSet. Cancels edit mode
   * and any edits made to the RowIterator.
   */
  ...
  /**
   * Will throw a DataSetException if not bound to a DataSet. Positions iterator
   * to row with values specified by rowLocate. Behaves much the same as
   * DataSet.locate().
   */
  public final boolean locate(ReadRow rowLocate, int locateOptions) /*-throws DataSetException-*/{
    ...
    synchronized (dataSet.getSyncObj()) {
      if (editing)
        _post();
      int rowFound = dataSet.dataSetStore.find(index, currentRow, rowLocate
          .getColumnList().getScopedArray(), rowLocate, locateOptions);
      if (rowFound < 0)
        return false;
      _goToRow(rowFound);
      return true;
  ...
  /**
   * Returns the {@link com.borland.dx.dataset.ReadWriteRow} this RowIterator is
   * bound to.
   * 
   * @return The {@link com.borland.dx.dataset.ReadWriteRow} this RowIterator is
   *         bound to.
   */
   ...
   * 
   * @return <b>true</b> if the data is being added to a new row in the DataSet
   *         or not.
   */
  ...
  private Index index;
  private final RowVariant value = new RowVariant();
