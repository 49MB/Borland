...
public class AggManager {
  ...
  /*-throws DataSetException-*/
  {
    ...
    for (int ordinal = 0; ordinal < count; ++ordinal) {
      ...
      if (column.lookup != null && column.lookup.isCalcField()) {
        if (calcs == null)
  ...
  private static boolean isAggregator(StorageDataSet dataSet, Column column,
      int ordinal)
  /*-throws DataSetException-*/
  {
  ...
  private static boolean addAggregator(Aggregator[] aggregatorMap,
      StorageDataSet dataSet, Column column, int ordinal)
  /*-throws DataSetException-*/
  {
  ...
  AggManager(StorageDataSet dataSet, boolean[] calcs,
      Aggregator[] aggregatorMap, int aggCount, Lookup[] lookupMap)
  /*-throws DataSetException-*/
  {
    this.calcs = calcs;
    Diagnostic.check(this.calcs != null);
    ...
    if (aggregatorMap != null) {
      if (dataSet.hasCalcAggFieldsListeners())
  ...
  /*-throws DataSetException-*/
  {
    ...
    if (dataSet.hasCalcAggFieldsListeners()) {
      initCustomAgg(row);
      for (CalcAggFieldsListener calcAggFieldsListener : dataSet.calcAggFieldsListeners)
        calcAggFieldsListener.calcAggAdd(customRow, dataSet.calcFieldsRow);
  ...
  /*-throws DataSetException-*/
  {
    ...
    if (dataSet.hasCalcAggFieldsListeners()) {
      initCustomAgg(row);
      for (CalcAggFieldsListener calcAggFieldsListener : dataSet.calcAggFieldsListeners)
        calcAggFieldsListener.calcAggDelete(customRow, dataSet.calcFieldsRow);
      updateCustomAggs(row);
    }
  }
  ...
  /*-throws DataSetException-*/
  {
    ...
    if (lookupMap != null && lookupMap[calcOrdinal] != null)
      ...
      aggregatorMap[calcOrdinal]
          .getVariant(internalReadRow, calcOrdinal, value);
  }
  ...
  private final InternalRow internalReadRow;
  private final boolean[] calcs;
  private final Lookup[] lookupMap;
  private final Aggregator[] aggregatorMap;
  private final Aggregator[] aggregators;
  private final StorageDataSet dataSet;
