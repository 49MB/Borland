 ...
 * writers; see the com.borland.dbswing source code for usage examples. Not for
 * general usage.
 * <p>
...
public class AccessEvent extends com.borland.jb.util.DispatchableEvent {
   ...
   * {@link com.borland.dx.dataset.DataSet#enableDataSetEvents(boolean)} is
   * called with an argument value of <b>true</b>.
   */
  ...
  /**
   * Reason for {@link #CLOSE} event. Indicates that DataSet was closed and it
   * is not known whether it will be opened again. dataSet.close() will cause
   * this event.
   */
  ...
  /**
   * Reason for {@link #CLOSE} event. Indicates that DataSet was closed and it
   * is expected to be reopened in a short time period. Usually called to
   * perform structural property change.
   */
  public static final int STRUCTURE_CHANGE = 8;
  
  /**
   * Reason for {@link #CLOSE} event. This is called when a non-structural
   * property change like Column Font or Color is changed. Also used when
   * DataSet.enableDataSetEvents(false) is called. Note that this event will not
   * be sent to DataSetViews listening to their associated StorageDataSets. In
   * practice, this event only makes its way to visual components that listen to
   * Access Events.
   */
   ...
   * Reason for {@link #CLOSE} event. Indicates that DataSet was closed and it
   * is expected to be reopened in a short time period. Usually called to
   * perform a sort order change.
   */
  public static final int SORT_CHANGE = 10;
   ...
   *          The event source.
   * @param id
   *          The event type: An {@link #OPEN} event is 1; a CLOSE event is 2.
   */
   ...
   *          The event source.
   * @param id
   *          The reason for the event. Reasons for {@link #OPEN} events
   *          include:
   *          <ul>
   *          <li>UNSPECIFIED = 1</li>
   *          <li>DATA_CHANGE = 2</li>
  ...
  public AccessEvent(Object source, int id, int reason) {
    this(source, id);
    Diagnostic.check(id == OPEN || id == CLOSE);
    this.reason = reason;
  }
  
  /**
   * Creates an internal event from the given source with the specified event
   * type and reason. The internal event drops the specified column.
   * 
   * @param source
   *          The event source.
   * @param id
  ...
  public AccessEvent(Object source, int id, int reason, Column dropColumn) {
    ...
    Diagnostic.check(reason == COLUMN_DROP);
    this.dropColumn = dropColumn;
  }
  
  /**
   * Creates an internal event from the given source with the specified event
   * type and reason. The internal event restructures the table.
   * 
   * @param source
   *          The event source.
   * @param id
   *          The event type: An OPEN event is a {@link #CLOSE} event is 2.
   * @param reason
   *          The reason for the event. Reasons for {@link #OPEN} events
   *          include:
   *          <ul>
   ...
   */
  public AccessEvent(Object source, int id, int reason, int oldOrdinal,
      int newOrdinal) {
    this(source, id, reason);
    Diagnostic.check(reason == COLUMN_MOVE);
   ...
   *          The event source.
   * @param id
   *          The event type: An OPEN event is 1; a {@link #CLOSE} event is 2.
   * @param reason
   *          The reason for the event. Reasons for {@link #OPEN} events
   *          include:
   *          <ul>
   ...
   */
  public AccessEvent(Object source, int id, int reason, Column oldColumn,
      Column newColumn) {
    this(source, id, reason);
    Diagnostic.check(reason == COLUMN_CHANGE);
  ...
  public void dispatch(final EventListener listener) {
    if (listener instanceof Component || listener instanceof DataSetAware
      ...
      DBUtilities.invokeOnSwingThread(new Runnable() {
        @Override 
        public void run() {
          ((AccessListener) listener).accessChange(AccessEvent.this);
        }
      });
    else
  ...
  /**
   * The event type. Valid return values for this method are defined as
  ...
  /**
   * Read-only property that returns the reason for the event. Valid values are
   * defined in the variables section of this class.
   * 
