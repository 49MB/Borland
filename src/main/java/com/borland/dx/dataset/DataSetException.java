...
/**
 * A base Exception class used often throughout the dataset package. The
 * <CODE>DataSetException</CODE> class extends the
 * <CODE>java.lang.Exception</CODE> class and is used heavily by the
 * <CODE>dataset</CODE> package. It encapsulates common errors that can be
 * generated, and methods necessary to access this error information.
 * 
 * 
 * <P>
 * The <CODE>DataSetException</CODE> class can have other types of exceptions
 * chained to them, for example, {@link java.io.IOException java.io.IOException}
 * and {@link java.sql.SQLException java.sql.SQLException} exceptions. In these
 * cases the <CODE>DataSetException</CODE> has an appropriate message that
 * describes the error from the perspective of the DataExpress level API.
 * 
 * <P>
 * Programmatically, use the <CODE>getExceptionChain()</CODE> to obtain any
 * chained exception. A chained exception (a singly linked list) can contain
 * non-<CODE>DataSetException</CODE> exceptions that were encountered at a lower
 * level API.
 * 
 * <!-- JDS start - remove next three paragraphs -->
 * 
 * <P>
 * dbSwing components automatically catch <CODE>DataSetExceptions</CODE> at
 * runtime, passing them to the {@link com.borland.dbswing.DBExceptionHandler
 * DBExceptionHandler} component for processing. If a dbSwing component performs
 * an operation that causes a <CODE>DataSetException</CODE> to occur, a
 * com.borland.dbswingDBExceptionDialog DBExceptionDialog} appears with a
 * message describing the error.
 * 
 * <P>
 * By default, the <CODE>DBExceptionDialog</CODE> has a "Stack Trace" button
 * that displays the exception stack trace. If the <CODE>DataSetException</CODE>
 * has chained exceptions, they can be viewed by pressing
 * <CODE>DBExceptionDialog</CODE>'s Previous and Next buttons. Refer to the
 * {@link com.borland.dbswing.DBExceptionDialog DBExceptionDialog} class for
 * information about customizing its default buttons.
 * 
 * <P>
 * If the exception thrown is a
 * {@link com.borland.dx.dataset.ValidationException ValidationException} (a
 * subclass of <CODE>DataSetException</CODE>), the
 * <CODE>DBExceptionDialog</CODE> displays only if there are no
 * <CODE>StatusEvent</CODE> listeners on the <CODE>DataSet</CODE>, for example,
 * a <CODE>JdbStatusLabel</CODE> or <CODE>JdbNavToolBar</CODE> component. A
 * <CODE>ValidationException</CODE> is generated by a constraint violation, for
 * example, a minimum or maximum value outside specified ranges, a data entry
 * that doesn't meet an edit mask specification, an attempt at updating a
 * read-only column, and so on. If a <CODE>JdbStatusLabel</CODE> is bound to a
 * <CODE>DataSet</CODE>, it automatically displays messages resulting from
 * constraint violations. <!-- JDS end -->
 * 
 * 
 */

public class DataSetException extends RuntimeException implements
    ChainedException {
  /**
   * 
   */
  private static final long serialVersionUID = 2276303934683227053L;
  protected int errorCode;
  protected ExceptionChain exceptionChain;
  
  /*
   * Error codes are reserved in ranges of 1024. Highest current range is
   * TransactionException-> 61024
   */
  ...
  /**
   * Mismatch between number of parameters markers in query and number of
   * parmaeters in ReadRow.
   */
  public static final int PARAMETER_COUNT_MISMATCH = BASE + 6;
  /**
   * Can't determine the table name for query property from metadata or from
   * parsing the query property.
   */
  ...
  /**
   * DataSet must have Columns before data can be imported. Temporary will
   * support eventually.
   */
  ...
  /**
   * Atempt to add a column with the same name, but different data type as an
   * existing DataColumn.
   */
  ...
  /**
   * Column already bound to a DataSet. Use (Column)Column.clone() to add this
   * column.
   */
  public static final int COLUMN_ALREADY_BOUND = BASE + 13;
  /**
   * Cannot change Column Data Type becuase the associated column already has
   * data.
   */
  ...
  /*
   * Cannot load more rows than what is specified by the dataSet maxRow
   * property. public static final int TOO_MANY_ROWS = BASE+17;
   */
  /**
  ...
  /**
   * Partial search option can only be used when last column searched on is of
   * String type".
   */
  ...
  /*
   * Query has not been executed yet public static final int QUERY_NOT_EXECUTED
   * = BASE+25;
   */
  ...
  /**
   * Cannot Load DataSet. DataSet not in load mode Unexpected Internal error.
   */
  ...
  /**
   * Cannot modify Column elements in a Scoped DataRow. Create a new scoped
   * DataRow if needed.
   */
   ...
   * DataRows must be created with the DataSet or DataSet they are used with. If
   * the structure of a DataSet changes, a new DataRow must be created for it.
   */
  public static final int INCOMPATIBLE_DATA_ROW = BASE + 31;
  /**
   * Attempt load or save a DataSet with an invalid DataSet DataFileFormat
   * property setting. See DataFileFormat for valid file format settings.
   */
  public static final int INVALID_DATA_FILE_FORMAT = BASE + 32;
  /**
   * The number and data type of columns in the DataSet MasterLinkColumns and
   * DetailLinkColumns must match
   */
  public static final int LINK_COLUMNS_ERROR = BASE + 33;
  /**
   * The SortDescriptor Keys property setting is not compatible with DataSet
   * LinkColumns property. The Keys property must start with all the columns
   * specified in the DataSet MasterLinkColumns and DetailColumns properties.
   */
  public static final int MASTER_DETAIL_VIEW_ERROR = BASE + 34;
  /*
   * <--------> The detail DataSet was opened directly. In a master/detail
   * relationship, the detail DataSet should be opened implicitly by opening a
   * dataSet on that DataSet.
   */
  // ! public static final int INVALID_DETAIL_DATASET_OPEN = BASE+35-recycled
  // below;
  /**
  ...
  /**
   * More than one row was affected by resolution query (either DELETE or UPDATE
   * query).
   */
  ...
  /**
   * QueryDataSets that use delayed detail fetching must have a where clause in
   * their query property.
   */
  public static final int NO_WHERE_CLAUSE = BASE + 40;
  /**
   * Invalid access on an empty DataSet. If you get this because a master
   * dataSet has no rows and the details are being fetched as needed, add
   * persistent columns to the detail dataset. This way the initialization logic
   * can at least determine what the columns are for the detail dataSet.
   */
  public static final int DATASET_HAS_NO_ROWS = BASE + 41;
  /**
   * Operation failed. DataSet is not open.
   */
  public static final int DATASET_HAS_NO_COLUMNS = BASE + 41; // (SS)
  /**
   * Operation failed. DataSet is not open.
   */
  ...
  /**
   * Operation failed. There is a CalcFieldsListener, but no Columns with
   * CalcType property set to CALC.
   */
  ...
  /**
   * Operation failed. AggDescriptor has no groupColumns or some groupColumns do
   ...
   * getExceptionChain(). printStackTrace will display all exceptions in the
   * chain.
   */
  ...
  /**
   * @updated JB3.0 Operation failed. Invalid Column data type for a DataSet
   *          Column. The data type is not supported for the
   *          StorageDataSet.Store setting in use (MemoryStore or DataStore). If
   *          the type used is ASSIGNED_NULL, then the cause is probably that a
   *          persistent Column was added to the DataSet without specifying the
   *          Column.DataType property. ASSIGNED_NULL is the default property
   *          setting when a Column is instantiated. It signifies an
   *          uninitialized state.
   */
  ...
  /**
   * @updated JB3.0 Detail DataSet being resolved without Master DataSet. You
   *          must use the Database.saveChanges() method that has a DataSet[]
   *          array as the first parameter. The array should include the detail
   *          and its associated master dataset. Note that the master dataSet in
   *          the array must be the same DataSet object instance as the one
   *          specified in the detail's MasterLink property.
   */
  ...
  /**
   * Trying to use Database to save changes to a DataSet with a
   * non-DatabaseResolver derived Resolver.
   */
  public static final int NOT_DATABASE_RESOLVER = BASE + 67;
  
  /**
   * Trying to perform an operation that requires a column name without having
   * specified one.
   */
  ...
  /**
   * column formatting error for default, min, max values using edit mask or
   * display format.
   */
  public static final int INVALID_FORMAT = BASE + 71;
  
  /**
   * @updated JB3.0 Attempting to read twice from a java.io.InputStream
   *          implementaiton that throws an exception when InputStream.reset()
   *          is called and InputStream.available() returns 0 bytes. To correct
   *          this, use an InputStream implementation that supports reset().
   */
   ...
   * being edited and the field value cannot be posted (most likely due to a
   * validation error). Unfortunately controls do not currently provide any
   * information on why a post failed. In future releases the actual exception
   * that stopped the field post will be correctly surfaced.
   */
  public static final int FIELD_POST_ERROR = BASE + 81;
  
  /**
   * The driver: {0} could not be loaded. This could be a problem with the
   * driver itself, or that the driver is not found on the classpath. In design
   * mode use the IDEClassPath, which is found in the file:
  ...
  /**
   * The driver: {0} could not be loaded. This could be a problem with the
   * driver itself, or that the driver is not found on the classpath. Modify the
   * classpath in the project properties.
   */
  public static final int DRIVER_NOT_LOADED_AT_RUNTIME = BASE + 83;
  
  /**
   * The URL: {0} could not be found. Check for misspellings, and that the right
   * driver is present on the classpath.
   */
  public static final int URL_NOT_FOUND = BASE + 84;
  
  /**
   * The URL: {0} could not be found. Check for misspellings, and that the right
   * driver is present on the classpath. In design mode use the IDEClassPath,
   * which is found in the file: "\JBuilder\bin\JBuilder.ini". Exit JBuilder
   * before editing this file.
   */
  public static final int URL_NOT_FOUND_IN_DESIGN = BASE + 85;
  
  /**
   * A column in the DetailLinkColumns should not be included in the
   * parameterRow. Column: {0}.
   */
  ...
  /**
   * The database property of the ProcedureDescriptors doesn't match the
   * database we are resolving to.
   */
  public static final int WRONG_DATABASE = BASE + 91;
  
  /**
   * Operation failed. There is a CalcAggFieldsListener, but no Columns with
   * CalcType property set to AGGREGATE.
   */
  ...
  /**
   * @since JB2.0 Cannot find detail with given detailName.
   */
  public static final int UNKNOWN_DETAIL_NAME = BASE + 98;
  
  /**
   * @since JB2.0 Class could not be found during deserialization process.
   */
  public static final int CLASS_NOT_FOUND_ERROR = BASE + 99;
  
  /**
   * @since JB2.0 When an updated row is loaded by StorageDataSet.loadRow(int
   *        status), the original row must be loaded immediately prior to the
   *        updated row.
   */
  public static final int NO_PRIOR_ORIGINAL_ROW = BASE + 100;
  
  /**
   * @since JB2.0 Store property setting is readOnly.
   */
  public static final int READ_ONLY_STORE = BASE + 101;
  
  /**
   * @since JB2.0 Store doesn't implement replaceStoreRow. Unexpected internal
   *        error.
   */
  ...
  /**
   * @since JB2.0 storeName property must be one or more characters in length.
   */
   ...
   *        Use the StorageDataSet.deleteDuplicates() method to delete the
   *        duplicates.
   */
  public static final int DELETE_DUPLICATES = BASE + 105;
  
  /**
   * @since JB3.0 For Columns with a JavaClass property setting. Checks that all
   *        values assigned to the column are of the same type as the
   *        Column.JavaClass property setting.
   */
  public static final int INVALID_CLASS = BASE + 106;
  
  /**
   * @since JB3.0 Prevents write access if StorageDataSet.CalcListener or
   *        StorageDataSet.AggCalcListener have not been set on a StorageDataSet
   *        component. Should only occur with StorageDataSets that use DataStore
   *        for a backing store. DataStore persists the class names of calc
   *        listeners. So if a StorageDataSet component opens without specifying
   *        a StorageDataSet.CalcListener or StorageDataSet.AggCalcListener that
   *        were used previously, the StorageDataSet is marked readOnly.
   * 
   *        To correct, 1) add the appropriate listeners or 2) call
   *        StorageDataSet.recalc().
   */
   ...
   * 
   * This is typically caused for the following reasons: 1) write operation
   * attempted while iterating a ReadRow 2) navigation, insertRow, or post()
   * operation attempted when not iterating a DataSet.
   */
  ...
  */
  ...
  public static final void throwExceptionChain(Throwable ex) /*-throws DataSetException-*/{
    DataSetException.throwException(DataSetException.EXCEPTION_CHAIN,
        Res.bundle.getString(ResIndex.ExceptionChain), ex);
  }
  
  /**
   * Creates and throws a DataSetException of type Exception.errorCode. Extracts
   * the corresponding String message for that error using
  ...
  static final void throwException(int errorCode, String message,
  ...
  /**
   * Creates a DataSetException object with properties as specified in its
   * parameters.
   * 
   * @param errorCode
   *          The error code generated.
  ...
  public DataSetException(int errorCode, String message, Throwable ex) {
    ...
    if (ex instanceof DataSetException)
      exceptionChain = ((DataSetException) ex).exceptionChain;
   ...
   *          The text message associated with the error code.
   * @param chain
   *          The ExceptionChain object that contains the linked list of
  ...
  /**
   * Overrides the base class implementation of printStackTrace() to display the
   * error code and any exceptions that may be chained to it.
   * 
   * @param out
   */
  @Override
  public void printStackTrace(java.io.PrintStream out) {
    out.println(Res.bundle.format(ResIndex.ErrorCode, getClass().getName(),
        Integer.toString(getErrorCode() % 1000)));
    if (exceptionChain == null)
  ...
  public void printStackTrace(java.io.PrintWriter s) {
    s.println(Res.bundle.format(ResIndex.ErrorCode, getClass().getName(),
        Integer.toString(getErrorCode() % 1000)));
    if (exceptionChain == null)
  ...
  /*
   * previous JavaDoc comments Add a listener for ExceptionListener dispatches.
   * If you want to ensure that this static reference is never garbage
   * collected, Create an object data member and set it to DataSet.class.
   * Example: Object classHolder = DataSet.class;
   */
  ...
  public static final void addExceptionListener(ExceptionListener listener) {
    DataSet.exceptionListeners = EventMulticaster.add(
        DataSet.exceptionListeners, listener);
  }
  ...
  public static final void removeExceptionListener(ExceptionListener listener) {
    DataSet.exceptionListeners = EventMulticaster.remove(
        DataSet.exceptionListeners, listener);
  }
  ...
  /**
   * Creates and throws a DataSetException of BAD_QUERY_PROPERTIES when one or
   * more properties of the QueryDescriptor are invalid.
   */
  public static final void badQueryProperties() /*-throws DataSetException-*/{
    throwException(BAD_QUERY_PROPERTIES,
        Res.bundle.getString(ResIndex.BadQueryProperties));
  }
  ...
  public static final void notSelectQuery() /*-throws DataSetException-*/{
    throwException(NOT_SELECT_QUERY,
        Res.bundle.getString(ResIndex.NotSelectQuery));
  }
  ...
  public static final void unexpectedEndOfQuery() /*-throws DataSetException-*/{
    throwException(UNEXPECTED_END_OF_QUERY,
        Res.bundle.getString(ResIndex.UnexpectedEndOfQuery));
  }
  ...
  public static final void unknownParamName(String columnName) /*-throws DataSetException-*/{
    throwException(UNKNOWN_PARAM_NAME,
        Res.bundle.format(ResIndex.UnknownColumnName, columnName));
  }
  
  /**
   * Creates and throws a DataSetException of
   * {@link #MISMATCHED_PARAMETER_FORMAT}.
   */
  public static final void mismatchedParameterFormat() /*-throws DataSetException-*/{
    throwException(MISMATCHED_PARAMETER_FORMAT,
        Res.bundle.getString(ResIndex.MismatchedParameterFormat));
  }
   ...
   */
  public static final void parameterCountMismatch(int paramCount,
      int paramRowCount, int paramMasterCount) /*-throws DataSetException-*/{
    throwException(PARAMETER_COUNT_MISMATCH, Res.bundle.format(
        ResIndex.ParameterCountMismatch, Integer.toString(paramCount),
        Integer.toString(paramRowCount), Integer.toString(paramMasterCount)));
  }
  
  static final void cantFindTableName() /*-throws DataSetException-*/{
    throwException(CANNOT_FIND_TABLE_NAME,
        Res.bundle.getString(ResIndex.CantFindTableName));
  }
  ...
  public static final void noUpdatableColumns() /*-throws DataSetException-*/{
    throwException(NO_UPDATABLE_COLUMNS,
        Res.bundle.getString(ResIndex.NoUpdatableColumns));
  }
  
  static final void cantImportNullDataSet() /*-throws DataSetException-*/{
    throwException(CANNOT_IMPORT_NULL_DATASET,
        Res.bundle.getString(ResIndex.CantImportNullDataSet));
  }
  ...
  public static final void unknownColumnName(String columnName) /*-throws DataSetException-*/{
    throwException(UNKNOWN_COLUMN_NAME,
        Res.bundle.format(ResIndex.UnknownColumnName, columnName));
  }
  
  static final void columnTypeConflict() /*-throws DataSetException-*/{
    throwException(COLUMN_TYPE_CONFLICT,
        Res.bundle.getString(ResIndex.ColumnTypeConflict));
  }
  
  static final void invalidColumnPosition() /*-throws DataSetException-*/{
    throwException(INVALID_COLUMN_POSITION,
        Res.bundle.getString(ResIndex.InvalidColumnPosition));
  }
  
  // ! static final void columnNotMember() /*-throws DataSetException-*/ {
  // ! throwException(COLUMN_NOT_MEMBER,
  // Res.bundle.getString(ResIndex.ColumnNotMember));
  // ! }
  static final void cannotChangeColumnDataType() /*-throws DataSetException-*/{
    throwException(CANNOT_CHANGE_COLUMN_DATA_TYPE,
        Res.bundle.getString(ResIndex.CannotChangeColumnDataType));
  }
  
  static final void setCalculatedFailure() /*-throws DataSetException-*/{
    throwException(SET_CALCULATED_FAILURE,
        Res.bundle.getString(ResIndex.SetCalculatedFailure));
  }
  ...
  public static final void unrecognizedDataType() /*-throws DataSetException-*/{
    throwException(UNRECOGNIZED_DATA_TYPE,
        Res.bundle.getString(ResIndex.UnrecognizedDataType));
  }
  
  // ! /*
  // ! static final void throwTooManyRows() /*-throws DataSetException-*/ {
  // ! throwException(TOO_MANY_ROWS,
  // Res.bundle.getString(ResIndex.TooManyRows));
  // ! }
  // ! */
  ...
  public static final void dataSetHasNoTable() /*-throws DataSetException-*/{
    throwException(DATASET_HAS_NO_TABLES,
        Res.bundle.getString(ResIndex.DataSetHasNoTable));
  }
  
  static final void columnNotInRow(String message) /*-throws DataSetException-*/{
    throwException(COLUMN_NOT_IN_ROW, message);
  }
  
  static final void throwEmptyColumnNames() /*-throws DataSetException-*/{
    throwException(EMPTY_COLUMN_NAMES,
        Res.bundle.getString(ResIndex.EmptyColumnNames));
  }
  
  static final void noNonBlobColumns() /*-throws DataSetException-*/{
    throwException(NO_NON_BLOB_COLUMNS,
        Res.bundle.getString(ResIndex.NoNonBlobColumns));
  }
  
  /**
   * Creates and throws DataSetException of
   * {@link #TRANSACTION_ISOLATION_LEVEL_NOT_SUPPORTED}.
   */
  public static final void transactionIsolationLevelNotSupported() /*-throws DataSetException-*/{
    throwException(TRANSACTION_ISOLATION_LEVEL_NOT_SUPPORTED,
        Res.bundle.getString(ResIndex.TransactionIsolationLevelNotSupported));
  }
  
  static final void partialSearchForString() /*-throws DataSetException-*/{
    throwException(PARTIAL_SEARCH_FOR_STRING,
        Res.bundle.getString(ResIndex.PartialSearchForString));
  }
  
  static final void needLocateStartOption() /*-throws DataSetException-*/{
    throwException(NEED_LOCATE_START_OPTION,
        Res.bundle.getString(ResIndex.NeedLocateStartOption));
  }
  ...
  public static final void queryInProcess() /*-throws DataSetException-*/{
    throwException(QUERY_IN_PROCESS,
        Res.bundle.getString(ResIndex.QueryInProcess));
  }
  
  static final void restructureInProgress(String name) /*-throws DataSetException-*/{
    throwException(RESTRUCTURE_IN_PROGRESS,
        Res.bundle.format(ResIndex.RestructureInProgress, name));
  }
  
  static final void throwAlreadyLoading() /*-throws DataSetException-*/{
    throwException(ALREADY_LOADING,
        Res.bundle.getString(ResIndex.AlreadyLoading));
  }
  
  static final void throwLoadingNotStarted() /*-throws DataSetException-*/{
    throwException(LOADING_NOT_STARTED,
        Res.bundle.getString(ResIndex.LoadingNotStarted));
  }
  
  static final void cannotUpdateScopedDataRow() /*-throws DataSetException-*/{
    throwException(CANNOT_UPDATE_SCOPED_DATA_ROW,
        Res.bundle.getString(ResIndex.CannotUpdateScopedDataRow));
  }
  
  static final void incompatibleDataRow() /*-throws DataSetException-*/{
    throwException(INCOMPATIBLE_DATA_ROW,
        Res.bundle.getString(ResIndex.IncompatibleDataRow));
  }
  
  static final void invalidDataFileFormat() /*-throws DataSetException-*/{
    throwException(INVALID_DATA_FILE_FORMAT,
        Res.bundle.getString(ResIndex.InvalidDataFileFormat));
  }
  
  static final void throwLinkColumnsError() /*-throws DataSetException-*/{
    throwException(LINK_COLUMNS_ERROR,
        Res.bundle.getString(ResIndex.LinkColumnsError));
  }
  
  static final void masterDetailViewError() /*-throws DataSetException-*/{
    throwException(MASTER_DETAIL_VIEW_ERROR,
        Res.bundle.getString(ResIndex.MasterDetailViewError));
  }
  
  // ! static final void invalidDetailDataSetOpen() /*-throws
  // DataSetException-*/ {
  // ! throwException(INVALID_DETAIL_DATASET_OPEN,
  // Res.bundle.getString(ResIndex.InvalidDetailDataSetOpen));
  // ! }
  static final void duplicateColumnName() /*-throws DataSetException-*/{
    throwException(DUPLICATE_COLUMN_NAME,
        Res.bundle.getString(ResIndex.DuplicateColumnName));
  }
  ...
  static final void resolveInProgress() /*-throws DataSetException-*/{
    throwException(RESOLVE_IN_PROGRESS,
        Res.bundle.getString(ResIndex.ResolveInProgress));
  }
   ...
   * @param message
   */
  public static final void noRowsAffected(String message) /*-throws DataSetException-*/{
    throwException(NO_ROWS_AFFECTED,
        Res.bundle.getString(ResIndex.NoRowsAffected) + message);
  }
   ...
   * @param message
   */
  public static final void multipleRowsAffected(String message) /*-throws DataSetException-*/{
    throwException(MULTIPLE_ROWS_AFFECTED,
        Res.bundle.getString(ResIndex.MultipleRowsAffected) + message);
  }
   ...
   */
  public static final void dataSetNotOpen(DataSet dataSet) /*-throws DataSetException-*/{
    Diagnostic.printStackTrace();
    throwException(
        DATASET_NOT_OPEN,
        Res.bundle.getString(ResIndex.DataSetNotOpen) + ": "
            + dataSet.toString());
  }
  
  static final void dataSetCorrupt(String name) /*-throws DataSetException-*/{
    throwException(DATASET_CORRUPT,
        Res.bundle.format(ResIndex.DataSetCorrupt, name));
  }
  ...
  static final void noCalcAggFields() /*-throws DataSetException-*/{
    throwException(NO_CALC_AGG_FIELDS,
        Res.bundle.getString(ResIndex.NoCalcAggFields));
  }
  
  /**
   * Creates and throws a DataSetException of CONNECTION_DESCRIPTOR_NOT_SET when
   * the connection properties associated with connecting to a Database are not
   * set. These properties are stored in the ConnectionDescriptor object.
   */
  public static final void connectionDescriptorNotSet() /*-throws DataSetException-*/{
    throwException(CONNECTION_DESCRIPTOR_NOT_SET,
        Res.bundle.getString(ResIndex.ConnectionDescriptorNotSet));
  }
  
  static final void invalidAggDescriptor() /*-throws DataSetException-*/{
    throwException(INVALID_AGG_DESCRIPTOR,
        Res.bundle.getString(ResIndex.InvalidAggDescriptor));
  }
  ...
  public static final void notSortable() /*-throws DataSetException-*/{
    throwException(INVALID_SORT_COLUMN,
        Res.bundle.getString(ResIndex.NotSortable));
  }
  ...
  public static final void resolveFailed(Exception ex) /*-throws DataSetException-*/{
    throwException(RESOLVE_FAILED,
        Res.bundle.getString(ResIndex.ResolveFailed), ex);
  }
  
  static final void missingResolver() /*-throws DataSetException-*/{
    throwException(MISSING_RESOLVER,
        Res.bundle.getString(ResIndex.MissingResolver));
  }
  ...
  public static final void notDatabaseResolver() /*-throws DataSetException-*/{
    throwException(NOT_DATABASE_RESOLVER,
        Res.bundle.getString(ResIndex.NotDatabaseResolver));
  }
  ...
  public static final void missingMasterDataSet() /*-throws DataSetException-*/{
    throwException(MISSING_MASTER_DATASET,
        Res.bundle.getString(ResIndex.MissingMasterDataSet));
  }
  ...
  /*-throws DataSetException-*/
  {
    throwException(
        INVALID_COLUMN_TYPE,
        Res.bundle.format(
            ResIndex.InvalidColumnType,
            new String[] { Variant.typeName(column.getDataType()),
  ...
  /*-throws DataSetException-*/
  {
    throwException(
        ...
        Res.bundle.format(ResIndex.InvalidColumnType,
            new String[] { Integer.toString(sqlType) }));
  }
  
  static final void nullColumnName()
  /*-throws DataSetException-*/
  {
    throwException(NULL_COLUMN_NAME,
        Res.bundle.getString(ResIndex.NullColumnName));
  }
  ...
  static final void cannotChangeColumn() /*-throws DataSetException-*/{
    throwException(CANNOT_CHANGE_COLUMN,
        Res.bundle.getString(ResIndex.CannotChangeColumn));
  }
  
  static final void invalidFormat(Exception ex, String columnName) /*-throws DataSetException-*/{
    throwException(INVALID_FORMAT,
        Res.bundle.format(ResIndex.InvalidFormat, new String[] { columnName }),
        ex);
  }
  ...
  public static final void noWhereClause(DataSet dataSet) /*-throws DataSetException-*/{
    throwException(
        NO_WHERE_CLAUSE,
        Res.bundle.format(ResIndex.NoWhereClause,
            new String[] { dataSet.getTableName() }));
  }
  
  static final void dataSetHasNoRows(DataSet dataSet) /*-throws DataSetException-*/{
    throwException(
        DATASET_HAS_NO_ROWS,
        Res.bundle.format(ResIndex.DataSetHasNoRows,
            new String[] { dataSet.getTableName() }));
  }
  
  static final void dataSetHasNoColumns(DataSet dataSet) /*-throws DataSetException-*/{
    throwException(
        DATASET_HAS_NO_COLUMNS,
        Res.bundle.format(ResIndex.DataSetHasNoColumns,
            new String[] { dataSet.getTableName() }));
  }
  ...
  public static final void onePassInputStream(Column column) /*-throws DataSetException-*/{
    throwException(
        ...
        Res.bundle.format(ResIndex.OnePassInputStream,
  ...
  public static final void insufficientRowId() /*-throws DataSetException-*/{
    throwException(INSUFFICIENT_ROWID,
        Res.bundle.getString(ResIndex.InsufficientRowId));
  }
  ...
  public static final void nonExistentRowId() /*-throws DataSetException-*/{
    throwException(NON_EXISTENT_ROWID,
        Res.bundle.getString(ResIndex.NonExistentRowId));
  }
  
  static final void refreshRowNotSupported() /*-throws DataSetException-*/{
    throwException(REFRESHROW_NOT_SUPPORTED,
        Res.bundle.getString(ResIndex.RefreshRowNotSupported));
  }
  
  static final void cannotSaveChanges(StorageDataSet dataSet) /*-throws DataSetException-*/{
    String tableName = dataSet.getTableName();
    throwException(CANNOT_SAVE_CHANGES, Res.bundle.format(
        ResIndex.CannotSaveChanges,
        new String[] { tableName != null ? tableName : "" })); // NORES
  }
  
  static final void cannotRefresh(StorageDataSet dataSet) /*-throws DataSetException-*/{
    String tableName = dataSet.getTableName();
    throwException(CANNOT_REFRESH, Res.bundle.format(ResIndex.CannotRefresh,
        new String[] { tableName != null ? tableName : "" })); // NORES
  }
  ...
  static final void invalidFormat(Exception ex, int row, Column column) /*-throws DataSetException-*/{
    throwException(
        INVALID_FORMAT,
        Res.bundle.format(ResIndex.InvalidFormatException, new String[] {
            Integer.toString(row), column.getColumnName() }), ex);
  }
  
  static final void columnAlreadyBound(DataSet dataSet, Column column) /*-throws DataSetException-*/{
    throwException(COLUMN_ALREADY_BOUND, Res.bundle.format(
        ResIndex.ColumnAlreadyBound, column.getColumnName(),
  ...
  public static final void connectionNotClosed(Exception ex) /*-throws DataSetException-*/{
    throwException(CONNECTION_NOT_CLOSED,
        Res.bundle.getString(ResIndex.ConnectionNotClosed), ex);
  }
  
  static final void fieldPostError(Exception ex) /*-throws DataSetException-*/{
    throwException(FIELD_POST_ERROR,
        Res.bundle.getString(ResIndex.FieldPostError), ex);
  }
  
  /**
   * Creates and throws a DataSetException of
   * {@link #DRIVER_NOT_LOADED_IN_DESIGN}.
   * 
   * @param driver
   */
  ...
  /*-throws DataSetException-*/
  {
    throwException(DRIVER_NOT_LOADED_IN_DESIGN,
        Res.bundle.format(ResIndex.DriverNotLoadedInDesign, driver));
  }
  
  /**
   * Creates and throws a DataSetException of
   * {@link #DRIVER_NOT_LOADED_AT_RUNTIME}.
   * 
   * @param driver
   */
  ...
  /*-throws DataSetException-*/
  {
    throwException(DRIVER_NOT_LOADED_AT_RUNTIME,
        Res.bundle.format(ResIndex.DriverNotLoadedAtRuntime, driver));
  }
  ...
  public static final DataSetException mkUrlNotFound(String url, Exception ex) {
    return new DataSetException(URL_NOT_FOUND, Res.bundle.format(
        ResIndex.UrlNotFound, url), ex);
  }
   ...
   * @return
   */
  public static final DataSetException mkUrlNotFoundInDesign(String url,
      Exception ex) {
    return new DataSetException(URL_NOT_FOUND_IN_DESIGN, Res.bundle.format(
        ResIndex.UrlNotFoundInDesign, url), ex);
  }
  ...
  {
    throwException(PROCEDURE_FAILED,
        Res.bundle.getString(ResIndex.ProcedureFailed), ex);
  }
  ...
  {
    throwException(PROVIDER_FAILED,
        Res.bundle.getString(ResIndex.ProviderFailed), ex);
  }
  ...
  public static final DataSetException providerOwned() {
    return new DataSetException(PROVIDER_OWNED,
        Res.bundle.getString(ResIndex.ProviderOwned));
  }
  
  /**
   * Creates and throws a DataSetException of BAD_PROCEDURE_PROPERTIES when one
   * or more properties of the ProcedureDescriptor are invalid.
   */
  public static final void badProcedureProperties() /*-throws DataSetException-*/{
    throwException(BAD_PROCEDURE_PROPERTIES,
        Res.bundle.getString(ResIndex.BadProcedureProperties));
  }
  
  static final void needStorageDataSetForFetchAsNeeded() /*-throws DataSetException-*/{
    throwException(NEED_STORAGEDATASET,
        Res.bundle.getString(ResIndex.NeedStorageDataSet));
  }
  ...
  public static final void mismatchParamResult() /*-throws DataSetException-*/{
    throwException(MISMATCH_PARAM_RESULT,
        Res.bundle.getString(ResIndex.MismatchParamResult));
  }
  ...
  public static final void procedureInProcess() /*-throws DataSetException-*/{
    throwException(PROCEDURE_IN_PROCESS,
        Res.bundle.getString(ResIndex.ProcedureInProcess));
  }
  ...
  public static final void needQueryProvider() /*-throws DataSetException-*/{
    throwException(NEED_QUERYPROVIDER,
        Res.bundle.getString(ResIndex.NeedQueryProvider));
  }
  ...
  public static final void needProcedureProvider() /*-throws DataSetException-*/{
    throwException(NEED_PROCEDUREPROVIDER,
        Res.bundle.getString(ResIndex.NeedProcedureProvider));
  }
  ...
  public static final void noDatabaseOnResolver() /*-throws DataSetException-*/{
    throwException(NO_DATABASE_TO_RESOLVE,
        Res.bundle.getString(ResIndex.NoDatabaseOnResolver));
  }
  ...
  public static final void unknownDetailName(String detailName) /*-throws DataSetException-*/{
    throwException(UNKNOWN_DETAIL_NAME,
        Res.bundle.format(ResIndex.UnknownDetailName, detailName));
  }
  ...
  static final void noOriginalRow() /*-throws DataSetException-*/{
    throwException(NO_PRIOR_ORIGINAL_ROW,
        Res.bundle.getString(ResIndex.NoOriginalRow));
  }
  ...
  public static final void readOnlyStore(String dataSetName) /*-throws DataSetException-*/{
    throwException(READ_ONLY_STORE,
        Res.bundle.format(ResIndex.ReadOnlyStore, dataSetName));
  }
  
  static final void missingReplaceRow() /*-throws DataSetException-*/{
    throwException(MISSING_REPLACESTOREROW,
        Res.bundle.getString(ResIndex.MissingReplaceRow));
  }
  
  /**
   * Creates and throws a DataSetException of {@link #INVALID_STORE_NAME} when
   * the value of the storeName property is blank.
   * 
   * @param name
   */
  public static final void invalidStoreName(String name) /*-throws DataSetException-*/{
    throwException(INVALID_STORE_NAME,
        Res.bundle.format(ResIndex.InvalidStoreName, name));
  }
  
  /**
   * Creates and throws a DataSetException of {@link #DELETE_DUPLICATES}.
   */
  public static final void deleteDuplicates() /*-throws DataSetException-*/{
    throwException(DELETE_DUPLICATES,
        Res.bundle.getString(ResIndex.DeleteDuplicates));
  }
   ...
   * @param javaClass
   */
  public static final void invalidClass(Class<?> javaClass) /*-throws DataSetException-*/{
    throwException(INVALID_CLASS,
        Res.bundle.format(ResIndex.DeleteDuplicates, javaClass.getName()));
  }
   ...
   * @param storeName
   */
  public static final void needsRecalc(String storeName) /*-throws DataSetException-*/{
    throwException(NEEDS_RECALC,
        Res.bundle.format(ResIndex.NeedsRecalc, storeName));
  }
  ...
  public static final void invalidClass(String storeClassName, String className) /*-throws DataSetException-*/{
    throwException(INVALID_STORE_CLASS, Res.bundle.format(
        ResIndex.InvalidStoreClass, storeClassName, className));
  }
  
  static final void invalidIteratorUse() /*-throws DataSetException-*/{
    throwException(INVALID_ITERATOR_USE,
        Res.bundle.getString(ResIndex.InvalidIteratorUse));
  }
  
  static final void duplicatePrimary() /*-throws DataSetException-*/{
    throwException(DUPLICATE_PRIMARY,
        Res.bundle.getString(ResIndex.DuplicatePrimary));
  }
  
  static final void noPrimaryKey() /*-throws DataSetException-*/{
    throwException(DUPLICATE_PRIMARY,
        Res.bundle.getString(ResIndex.NoPrimaryKey));
  }
  
  static final void invalidSortAsInserted() /*-throws DataSetException-*/{
    throwException(INVALID_SORT_AS_INSERTED,
        Res.bundle.getString(ResIndex.InvalidSortAsInserted));
  }
  ...
  static final void invalidConstraintName(String name) /*-throws DataSetException-*/{
    throwException(INVALID_CONSTRAINT_NAME,
        Res.bundle.format(ResIndex.ConstraintMissing, name));
  }
  
  public static final void tooManyAutoInc(Column col1, Column col2) /*-throws DataSetException-*/{
    throwException(
        TOO_MANY_AUTO_INCREMENTS,
        Res.bundle.format(ResIndex.TooManyAutoInc, col1.getColumnName(),
            col2.getColumnName()));
  }
  ...
  static final void dependentForeignKey(String name1, String name2) /*-throws DataSetException-*/{
    throwException(
        ...
        Res.bundle.format(ResIndex.InvalidConstraintName, new Object[] { name1,
            name2 }));
  }
