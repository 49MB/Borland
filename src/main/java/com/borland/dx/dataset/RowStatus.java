...
/**
 * All row in a dataSet have status. Status settings are used by DataSets to
 * filter. Status settings are used recording edit state. This is used when the
 * row data must be saved back to its source. For a QueryDataSet, the source
 * would be its associated JDBC driver.
 */
...
public interface RowStatus {
  ...
  /**
   * Row was loaded (ie from the execution of a QueryDataSet's JDBC query or an
   * import operation).
   */
  ...
  public static final int CALCULATED = 0x20;
  ...
  /**
   * SS: Dynamic Calcfields+Lookups.
   */
  public static final int FIELDS_CALCULATED = 0x400;
  
  // Do not make public. Use public definitions in DataConst (formerly in
  // MatrixData).
  //
  /**
   * This variable is used internally by other com.borland classes. You should
  ...
  static final int END_LOADING = -1;
  static final int EARLY_BREAK_LOADING = -2;
}
