 ...
 * 
 * I choose to use the one from the JBuilder Reference pages that had correct
 * HTML tags [asc]
 * 
 * 
 * The PickListDescriptor describes a pick list relationship between a Column of
 * one DataSet and a second DataSet that provides values from which you can
 * choose. To Access the PickListDescriptor in the JBuilder Component Inspector,
 * set the picList property of a Column component.
 * 
 * The DataSet that contains the items to display in the pick list is specified
 * in the pickListDataSet property. The pickListDisplayColumns property
 * describes the columns of that DataSet that are shown in the pick list.
 * 
 ...
 * PickListItemEditor as its itemEditor by default. JBCL model-view controls
 * that automatically make use of the Column PickList property include:
 * 
 * GridControl ListControl FieldControl
 * 
 ...
 * PopupPickListItemEditor is a component available on the JBCL tool palette
 * page.
 * 
 * Once a row is selected, the values of the pickListColumns in the
 * pickListDataSet are copied into the destinationColumns of this DataSet .
 * 
 * In JBuilder 2.0 the lookupDisplayColumn property was added. This is an
 * optional property setting that specified the name of the column in the
 * pickListDataSet to display values from.
 * 
 * There are three basic ways to configure the PickList property (number 1 and 2
 * will probably be of most value):
 * 
 * 1) Simple "fill in". pickList editor for visual control that provides
 * selection of values from a pickListDataSet. The selected values are filled
 * into one or more column values.
 * 
 * For this configuration set the pickListDataSet, destinationColumns (columns
 * to copy to), pickListColumns (columns to copy from the pickListDataSet), and
 * pickListDisplayColumns (columns values to display from the pickListDataSet
 * when the pickList Column is being edited)
 * 
 * 2) "Fill in" and "lookup" display. Same as #1, except you can specify a
 * lookupDisplayColumn from the pickListDataSet. This allows you to store a
 * "code number" code in the column, but display a "code description".
 * 
 * The values from the lookupDisplayColumn in the pickListDataSet will be
 * displayed instead of the value that is stored in the pickList Column. The
 * value to display is chosen by using the current values in destinationColumns
 * to locate the row in the pickListDataSet that has the same values for its
 * pickListColumns. If a match is found, the lookupDisplayColumn value for that
 * row in the pickListDataSet is used for display. Note that the DataSet.set/get
 * value methods will retrieve and set the "real" value stored. To retrieve the
 * display value for such a column, use the DataSet.getDisplayVariant() method.
 * 
 ...
 * configuration #1, this configuration will display the code description from
 * the pickListDataSet. So if the code description is changed in your
 * pickListDataSet, that change will be refleted in your DataSet since this
 * display value is always "looked up". In configuration #1, if the code
 * description changes in the pickListDataSet, your dataSet will still show the
 * old "fill in" values.
 * 
 ...
 * #2. The only difference is that the lookup column cannot be one of the
 * columns listed in the destinationColumns property.
 */

public class PickListDescriptor implements java.io.Serializable {
  
  /**
   * Constructs a PickListDescriptor object with the properties stated in its
   * parameters.
   * 
   * @param pickListDataSet
   *          The DataSet object that contains the data for display in the pick
   *          list.
   * @param pickListColumns
   *          The columns of the pickListDataSet from which values in the
   *          selected row are copied to the destinationColumns.
   * @param pickListDisplayColumns
   *          The Column components of the DataSet to display in the pick list.
   *          This property is expressed as an array of String names.
   * @param destinationColumns
   *          The Column components that are populated with the values
   *          associated with the selected pick list choice. This property is
   *          expressed as an array of String column names.
   * @param lookupDisplayColumn
   *          The name of the Column in the picklistDataSet to display the
   *          column whose pickList property is being set.
   * @param enforceIntegrity
   *          Whether to enforce data integrity rules (data constraints) on the
   *          data added to the destinationColumns.
   */
  public PickListDescriptor(DataSet pickListDataSet, String[] pickListColumns,
      String[] pickListDisplayColumns, String[] destinationColumns,
      String lookupDisplayColumn, boolean enforceIntegrity) {
    ...
    if (lookupDisplayColumn == null)
      this.lookupDisplayColumn = pickListDisplayColumns[0];
    else
   ...
   * @param pickListDisplayColumns
   *          The Column components of the DataSet to display in the pick list.
   *          This property is expressed as an array of String column names. If
   *          your pick list contains multiple columns, you must also set the
   *          itemEditor property of the Column to an instance of the
   *          PopupPickListItemEditor. The default pick list item editor is the
   *          PickListItemEditor, which can only display a picklist with a
   *          single column.
   * @param destinationColumns
   *          The Column components that are populated with the values
   *          associated with the selected pick list choice. This property is
  ...
  public PickListDescriptor(DataSet pickListDataSet, String[] pickListColumns,
    ...
    this(pickListDataSet, pickListColumns, pickListDisplayColumns,
        destinationColumns, null, enforceIntegrity);
  }
  ...
  /**
   * @since JB2.0 Name of column in Picklist DataSet to display for this column
   *        by data aware controls.
   */
  public final String getLookupDisplayColumn() {
    return lookupDisplayColumn;
  }
  ...
  private final DataSet pickListDataSet;
  private final String[] pickListColumns;
  private final String[] pickListDisplayColumns;
  ...
  public String toString() {
    String res = "PickListDataSet=" + pickListDataSet.toString() + "\n"
        + "destinationColumns=";
    for (String s : destinationColumns)
      res += s + " ";
    res += "\n";
    res += "picklistColumns=";
    for (String s : pickListColumns)
      res += s + " ";
    res += "\n";
    return res;
  }
