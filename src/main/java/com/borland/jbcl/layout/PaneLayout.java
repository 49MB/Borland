 ...
 *
 * For a UI to modifify proportions, use com.borland.jbcl.control.SplitPanel.
 *
 * Further work:
...
public class PaneLayout implements LayoutManager2, Serializable
{
  ...
  public void layoutContainer(Container parent) {
    ...
    if (rootNode != null) {
      ...
      Insets insets = parent.getInsets();
      Rectangle location = new Rectangle(insets.left, insets.top, d.width -insets.left -insets.right, d.height -insets.top -insets.bottom);
  ...
  void addChild(String name,String splitComponentName, String position, Component newComponent, float proportion) {
    if (rootNode == null) {
      ...
      if (splitComponentName == null || splitComponentName.length() == 0) {
        if (position == null)
          rootNode.addChild(name,newComponent,null,proportion);
        else
        //System.err.println("split with " + lastComponentAdded + " instead " + position);
    ...
    if (constraints instanceof PaneConstraints) {
      ...
      //System.err.println("ignored add for " + ((PaneConstraints)constraints).name);
       ...
       if (((PaneConstraints)constraints).name == null)   {
        addCount++;
        String name = "component" + addCount; 
        //addChild(newComponent,0.5f);     //given a useless constraint, so throw it away
       }
       // if (((PaneConstraints)constraints).splitComponentName == null) {
        //the constraint is semi-useless, so use the name and throw away everything else
       // if (rootNode == null)
       //   rootNode = new PaneNode(((PaneConstraints)constraints).name, newComponent, PaneConstraints.ROOT);
       // else
       //   rootNode.addChild(((PaneConstraints)constraints).name, newComponent, null, 0.5f);  //NORES
        //lastComponentAdded = ((PaneConstraints)constraints).name;
      //}
      //else
      if (setComponentConstraints(newComponent,(PaneConstraints)constraints)) { //make sure it has not already been added
  ...
  boolean justDeleted(Component newComponent, PaneConstraints constraints) {
    if (lastDeletion == null || constraints == null)
      return false;
    if (lastDeletion.childComponent == newComponent) {
      if (lastDeletion.childNodeA == null) {
        ...
        if ((constraints.splitComponentName != null && constraints.splitComponentName.equals(node.getNodeAComponent())) ||
          ...
          node.widthDivide = lastDeletion.widthDivide;
          node.heightDivide = lastDeletion.heightDivide;
          node.reverse = lastDeletion.reverse;
          node.horizontal = lastDeletion.horizontal;
          lastDeletion = null;
        ...
        //System.err.println(constraints.splitComponentName +"," + node.getNodeAComponent());
        //TODO: verify that split component constraint did not change
...
class PaneNode implements Serializable
{
  ...
  public PaneNode(String childName,Component child, String position) {
    ...
    widthDivide = 1.0f;
    heightDivide = 0.5f;
    horizontal = position.equals(PaneConstraints.TOP) || position.equals(PaneConstraints.BOTTOM);  
    reverse = position.equals(PaneConstraints.TOP) || position.equals(PaneConstraints.LEFT);  
  }
  ...
  void dump() {
    Diagnostic.println("childComponent:" + childComponent);   
    Diagnostic.println("childNodeA:" + childNodeA);           
    Diagnostic.println("childNodeB:" + childNodeB);           
  }
  ...
  public void addChild(String childName, Component child, String position, float proportion) {
    ...
    if (childComponent != null) {
      ...
      else if (position.equals(PaneConstraints.RIGHT) || position.equals(PaneConstraints.BOTTOM))
        reverse = false;
      else  {
        reverse = true;
        if (!position.equals(PaneConstraints.LEFT) && !position.equals(PaneConstraints.TOP) && !position.equals(PaneConstraints.ROOT))
          proportion = 0.5f;
      }
  ...
  public PaneNode removeChild(Component child) {
    ...
    if (childNodeA.childComponent == child) {
      ...
      rc = childNodeA;
      rc.widthDivide = widthDivide;
      rc.heightDivide = heightDivide;
      rc.reverse = reverse;
  ...
  private void writeObject(ObjectOutputStream s) throws IOException {
    ...
    s.writeObject(childComponent instanceof Serializable ? childComponent : null);
  }
