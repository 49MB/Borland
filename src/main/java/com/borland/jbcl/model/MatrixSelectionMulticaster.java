...
/**
 * A multicaster for MatrixSelectionEvents.
 */
public class MatrixSelectionMulticaster implements MatrixSelectionListener
{
      ...
      for (int index = 0; index < count; ++index) {
        ...
        switch (e.getID()) {
          ...
          case SelectionEvent.RANGE_CHANGE:
            listenersCopy[index].selectionRangeChanged(e);
            break;
  ...
  /**
   * Simple list management that avoids synchronized/functional interface of Matrix.  Key to this
   ...
   */
  public int find(MatrixSelectionListener listener) {
    if (listeners != null ) {
   ...
   *
   */
  public synchronized final void add(MatrixSelectionListener listener) {
    if (find(listener) < 0) {
      MatrixSelectionListener[] newListeners;

      if (listeners == null)
        newListeners = new MatrixSelectionListener[1];
      else {
        newListeners = new MatrixSelectionListener[listeners.length+1];
   ...
   *
   */
  public synchronized final void remove(MatrixSelectionListener listener) {
    ...
    if (index > -1) {
      ...
      if (listeners.length == 1)
        ...
        MatrixSelectionListener[] newListeners = new MatrixSelectionListener[listeners.length-1];
