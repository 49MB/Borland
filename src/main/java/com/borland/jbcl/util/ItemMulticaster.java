...
public class ItemMulticaster implements ItemListener
{
  ...
  public final void dispatch(ItemEvent e) {
    ...
    if (listenersCopy != null) {
      ...
      for (int index = 0; index < count; ++index) {
        ...
        listenersCopy[index].itemStateChanged(e);
   ...
   */
  public int find(ItemListener listener) {
    if (listeners != null ) {
   ...
   *
   */
  public synchronized final void add(ItemListener listener) {
    if (find(listener) < 0) {
      ItemListener[] newListeners;

      if (listeners == null)
        newListeners = new ItemListener[1];
      else {
        newListeners = new ItemListener[listeners.length+1];
   ...
   *
   */
  public synchronized final void remove(ItemListener listener) {
    ...
    if (index > -1) {
      ...
      if (listeners.length == 1)
        ...
        ItemListener[] newListeners = new ItemListener[listeners.length-1];
