...
public class KeyMulticaster implements KeyListener
{
   ...
   */
  public int find(KeyListener listener) {
    if (listeners != null ) {
   ...
   *
   */
  public synchronized final void add(KeyListener listener) {
    if (find(listener) < 0) {
      KeyListener[] newListeners;

      if (listeners == null)
        newListeners = new KeyListener[1];
      else {
        newListeners = new KeyListener[listeners.length+1];
   ...
   *
   */
  public synchronized final void remove(KeyListener listener) {
