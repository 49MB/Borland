...
/**
 * The Diagnostic component collects useful diagnostic functions for debugging
 * program flow and output.
 * <p>
 * All calls to <code>Diagnostic</code> methods with a <strong>void</strong>
 * return type can be removed from the compiled classes by using the compiler's
...
public class Diagnostic {
   ...
   */
  public static boolean outputEnabled = (System.getProperty(
      "jb.util.diagnostic", "off").equals("on"));
  
  /**
   * Enables or disables output of diagnostic messages to
  ...
  private static int outLevel = parseInt((System.getProperty(
      "jb.util.trace.level", "0")));
  private static boolean checking = true;
  private static Vector<Object> traceCategories = initTraceCategories();
  
  /*
   * !doc Not sure whinch example is better.
   * 
   * Diagnostic.println(++Diagnostic.count+"\tdebug message"); or (same output)
   * Diagnostic.printlnc("debug message");
   */

  /**
   * A common counter variable used used to add line numbers to debug output
   * messages. For example:
   * 
   * <PRE>
   * 
   * &lt;CODE&gt;Diagnostic.out.println(++Diagnostic.count+&quot;\tdebug message&quot;);&lt;/CODE&gt;
   * 
   * </PRE>
   */
  ...
  /**
   * Enables or disables all output of diagnostic messages to
   * <code>System.err</code>.
   */
  ...
  /**
   * Explicitly sets the stream to which diagnostic messages should be sent.
   * 
   * @param log
   ...
   * <p>
   * <strong>Note:</strong> This does not affect direct access to
   * <code>out</code>, nor does it affect output in the checking functions. Use
   * <code>enableOutput()</code> instead.
   * 
   * @param level
   *          The trace level. <code>0</code> is the highest level and
   *          <code>+maxint</code> is the lowest level. Setting this level to -1
  ...
  /**
   * Gets the minimum threshold for trace and warning output. <code>0</code> is
   * highest level and <code>+maxint</code> is lowest level.
   * 
   ...
   * 
   * <P>
   * To set up a category-based tracing or warning, pass in a unique <CODE>
     * String</CODE>, <CODE>Class</CODE> or other object that supports a meaningful
   * <CODE>toString</CODE> operation. When a call to a trace or warn method that
   * takes a category is made (for example, <CODE>trace(Object
   * category, String description)</CODE>), the trace is displayed if an <CODE>
     * addTraceCategory()</CODE>
  ...
  /**
   * Checks a condition. Same as {@link #check(boolean, java.lang.Object)}, but
   * typically placed at the start of the method body.
   * 
   * @param condition
  ...
  /**
   * Checks a condition. Same as {@link #check(boolean)}, but typically placed
  ...
  /**
   * Check a condition within a method body. Use this to describe assumed
   * results and state after internal operations. A check is raised if the given
   * condition is not true.
   * 
   * <p>
   * An error here usually indicates an internal problem with the class.
   */
  public static void check(boolean condition, Object description)
      throws IllegalStateException {
    if (checking && !condition) {
      throw new IllegalStateException(Thread.currentThread()
          + " check failed: " + description); // NORES
    }
  }
  
  /**
   * Checks a condition within a method body. It throws an
   * <CODE>IllegalStateException</CODE> if the given condition is
   * <STRONG>false</STRONG>.
   * 
   * @param condition
  ...
  /**
   * Calls <code>check(false)</code> to force a failure but prints the exception
   * message on the stack trace first.
   * 
   * @param ex
  ...
  /**
   * Outputs a warning if the threshold level is high enough, and a condition is
   * <b>true</b>, and general output is enabled.
   * 
   * @param level
   *          The trace level. <code>0</code> is highest level and
   *          <code>+maxint</code> is lowest level. Setting this level to -1
   *          effectively turns off traces and warnings.
   * @param condition
   *          The boolean condition, either <b>true</b> or <b>false.</b>
   * @param description
   *          The string to trace.
  ...
  public static void warn(int level, boolean condition, String description) {
    if (condition && level <= outLevel)
      println("wn[" + level + "] " + indent.substring(0, level * 2)
          + description); // NORES
  }
  
  /**
   * Outputs a warning if the category object is enabled, the boolean condition
   * is <b>true</b>, and general output is enabled.
   * 
   * @param category
   ...
   *          The string to trace.
   */
  public static void warn(Object category, int level, boolean condition,
      String description) {
  ...
  /**
   * Outputs a trace if the threshold level is high enough and general output is
   * enabled.
   * 
   * @param level
   *          The trace level. <code>0</code> is highest level and
  ...
  public static boolean isTrace(Object category) {
    return traceCategories.contains(category);
  }
  
  /**
   * Outputs a trace if the category and general output are both enabled, and
   * the threshold level is high enough.
   * 
   * @param category
   *          The category object to trace.
   * @param level
  ...
  public static void trace(Object category, int level, String description) {
    if (level <= outLevel && traceCategories.contains(category))
      println("[" + category.toString() + "." + level + "] "
          + indent.substring(0, level * 2) + description);
  }
  
  /**
   * Print a diagnostic stack trace of the current thread to the diagnostic out
   * stream
   */
  ...
  /**
   * Prints a diagnostic stack trace of the current thread to the diagnostic out
   ...
   * 
   * @param condition
   *          Only print if this condition is true.
   * @param message
  ...
  /**
   * Prints a message to the diagnostic out stream; each line is preceded by a
   * line number (incremented count).
   * 
   * @param message
   *          The message to print.
   */
  ...
  /**
   * Prints a message to the diagnostic out stream; each lines is preceded by a
   * line number (incremented count) and a tab character.
   * 
   * @param message
   *          The message to print.
   */
  ...
  /*
   * Convert a String to an intager. Returns 0 if it cannot parse the String
   */
  ...
  /*
   * Parse the comma seperated list of trace categories and add each category to
   * the Vector of traceCategories
   */
  private static final char sep = ',';
  
  private static Vector<Object> initTraceCategories() {
  ...
  // ! WARNING! This is a bad method since it references another borland
  // package.
  // ! Note that this source file is compiled with Diagnostics enabled, so if
  // another
  // ! dependent package is compiled implicitly, it will be compiled with
  // Diagnostics enabled!!!
  // ! See bug 13536
  // !
  // ! /**
  ...
  // ! for (int i = 0; i < com.borland.jbcl.model.ItemPainter.states.length;
  // i++) {
  // ! if ((state & com.borland.jbcl.model.ItemPainter.states[i]) != 0)
  // ! stateText += "|" + com.borland.jbcl.model.ItemPainter.stateNames[i]; //
  // NORES
  // ! }
  ...
  // ! }
  // private static OrderedSet traceCategories = new OrderedSet(); // more
  // overhead, but faster with many items
}
