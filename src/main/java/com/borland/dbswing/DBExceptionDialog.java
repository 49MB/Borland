...
public class DBExceptionDialog extends JDialog
  implements ActionListener, KeyListener, ExceptionDialog
{
      ...
      try {
        if (closeDataStores &&
          ...
          if (store != null &&
            ...
            isOpenMethod = store.getClass().getMethod("isOpen", (Class<?>[])null);
            isOpen = ((Boolean) isOpenMethod.invoke(store,(Object[])null)).booleanValue();
            if (isOpen) {
              closeMethod = store.getClass().getMethod("close", (Class<?>[])null);
              closeMethod.invoke(store, (Object[])null);
            }
          }
        }
