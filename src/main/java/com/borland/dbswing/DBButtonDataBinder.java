...
//            compareVariant.setFromString(columnAwareSupport.getColumn().getDataType(), unselectedValue);
        ...
        else if (mode == CLEAR_VALUE &&
            columnAwareSupport.dataSet.isEditing() &&
            !columnAwareSupport.isNull()) {
  ...
  */
  public void dataChanged(DataChangeEvent event) {
    ...
    boolean affectedOurRow = ((affectedRow == columnAwareSupport.dataSet.getRow()) ||
      affectedRow == -1) && event.isColumnAffected(columnAwareSupport.columnName);
    if (affectedOurRow) {
      updateButtonState();
    }
  }
  ...
  private void bindColumnProperties() {
    ...
    if (button != null && button.isDisplayable()) {
      ...
      if (columnAwareSupport.isValidDataSetState()) {
        ...
        buttonModel.setEnabled(getDataSet().isEditable() && column.isEditable() && !column.isReadOnly());
      }
    }
  }
