 ...
 * </p>
 */
public class DBLabelSupport extends DBColumnAwareSupport implements
  ...
  public void setColumnNameIcon(String columnNameIcon) {
    if (this.columnNameIcon == null
        || !this.columnNameIcon.equals(columnNameIcon)) {
      ...
      if (dataSetChanged || columnChanged) {
        ...
        if (dataSet != null) {
          ...
          if (columnNameIcon != null) {
            try {
              columnOrdinalIcon = dataSet.getColumn(columnNameIcon)
                  .getOrdinal();
              if (!(dataSet.getColumn(columnNameIcon).getDataType() == Variant.INPUTSTREAM || dataSet
                  .getColumn(columnNameIcon).getDataType() == Variant.OBJECT)) {
  ...
  public void putIcon(InputStream inputStream) {
    try {
      if (isValidDataSetState() && columnOrdinalIcon != -1) {
        ...
        if (bytes == null)
          dataSet.setAssignedNull(columnOrdinalIcon);
        else
          dataSet.setInputStream(columnOrdinalIcon, new InputStreamToByteArray(
              bytes));
      }
    } catch (DataSetException e) {
      DBExceptionHandler.handleException(dataSet, e);
      ...
      if (isValidDataSetState() && columnOrdinalIcon != -1) {
        ...
        if (v.getType() == Variant.INPUTSTREAM) {
          ...
          if (bmpImage != null) {
            ...
            // if the label was bound to the INPUTSTREAM column, assume it
            // contains
