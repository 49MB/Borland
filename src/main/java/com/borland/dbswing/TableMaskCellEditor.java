...
public class TableMaskCellEditor extends JTextField implements TableCellEditor,
    DocumentListener, ActionListener, CustomPaintSite, java.io.Serializable {
    ...
    if (ems != null) {
      ...
      if (value != null) {
        if (value instanceof String)
          ...
          if (formatter == null) {
            VariantFormatStr vfs = new VariantFormatStr(null, type);
            variantValue.setAsObject(value, type);
            setText(vfs.format(variantValue));
          } else {
            variantValue.setAsObject(value, type);
            setText(formatter.format(variantValue));
          }
        }
      } else {
        setText("");
  ...
  private void textModified(DocumentEvent e) {
    if (!ignoreModelChange && !touched && dbTableModel != null) {
      try {
        ...
        // later
        dbTableModel.getDataSet()
            .startEdit(
                dbTableModel.getColumn(table
    ...
    if (ems != null) {
      ...
      try {
        ...
        if (ex instanceof InvalidFormatException) {
          state.cursorPos = ((InvalidFormatException) ex).getErrorOffset(); // error
          // adjusts
          // cursor
        }
        updateSelection(); // and shows it to user
        try {
          ValidationException.invalidFormat(ex, dbTableModel.getColumn(table
              .convertColumnIndexToModel(columnIndex)), ex.getMessage());
      ...
      if (formatter != null) {
        try {
          ...
          Column column = dbTableModel.getColumn(table
              .convertColumnIndexToModel(columnIndex));
          try {
            /*
             * if (formatter.getFormatObj() instanceof java.text.DateFormat) {
             * oldPattern = formatter.getPattern(); int yearMaskIndex = -1; if
             * ((yearMaskIndex = DBUtilities.yearMaskPos(oldPattern)) != -1) {
             * int yearMaskCount = 1; int patternLength = oldPattern.length();
             ...
             * + yearMaskCount < patternLength) ?
             * oldPattern.substring(yearMaskIndex + yearMaskCount) : ""); } } if
             * (widePattern != null) { formatter.setPattern(widePattern); } }
             */
            formatter.parse(getText(), variantValue);
            dbTableModel.validate(column, variantValue);
          } catch (Exception e) {
            ValidationException.invalidFormat(e, column, e.getMessage());
          } finally {
            if (widePattern != null) {
          ...
          ValidationException.invalidFormat(ex, dbTableModel.getColumn(table
              .convertColumnIndexToModel(columnIndex)), ex.getMessage());
          // } catch (ValidationException ex2) {
          // DBExceptionHandler.handleException(dbTableModel.getDataSet(), this,
          // ex2);
          // }
          return false;
        }
  ...
  public void reset() {
    ...
    setFont(defaultFont);
    setHorizontalAlignment(DBUtilities.convertJBCLToSwingAlignment(
  ...
  protected void processKeyEvent(KeyEvent e) {
    if (e.getKeyCode() == KeyEvent.VK_TAB
        || (e.getKeyCode() == KeyEvent.VK_ENTER && !e.isControlDown() && e
      ...
      if (e.isShiftDown()) {
        ActionListener a = table.getActionForKeyStroke(KeyStroke.getKeyStroke(
            KeyEvent.VK_TAB, ActionEvent.SHIFT_MASK));
        if (a != null)
          a.actionPerformed(tabEvent);
      } else {
        ActionListener a = table.getActionForKeyStroke(KeyStroke.getKeyStroke(
            KeyEvent.VK_TAB, 0));
        if (a != null)
        ...
        && e.getID() == KeyEvent.KEY_TYPED && !e.isControlDown()) {
      if (isValidValue()) {
        fireEditingStopped();
        e.consume();
        return;
  ...
  private void handleKeyEvent(KeyEvent e) {
    ...
    if (!acceptKeyReleasedEvent && id != KeyEvent.KEY_PRESSED
    ...
    default:
      ...
      if (!acceptKeyReleasedEvent && id != KeyEvent.KEY_TYPED || ch >= 0
          && ch < ' ') {
        if (ch == '\b') {
          e.consume();
        }
